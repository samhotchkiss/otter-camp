...
- ISSUE #2 (AMBIGUOUS): Doc 04 describes a stale 7-step bootstrap sequence. Doc 14's 10-step sequence is authoritative. When reading doc 04, ignore its bootstrap section entirely. The correct steps are in doc 14.
...
- ISSUE #4 (AMBIGUOUS): doc 14's bootstrap dataset section says all staff agents have `private_memory: enabled`, but doc 05 body text limits this to staff PMs, Frank, Lori, and Ellie. Until Sam resolves this, implement per doc 05 body text (only the named four get `private_memory=true`). When the starter trio stub in step 7 is replaced by the real implementation in task 013/014, use the same rule.
- ISSUE #24 (GAP): Starter trio profile update on OtterCamp version upgrade is partially unresolved. When the bootstrap detects an existing installation (step 2 finds existing org), update `system_prompt` for the starter trio agents if the current app version differs from the version recorded in the step 10 audit event metadata. Do NOT overwrite `operator_instructions`. For all other fields (tool policy, model assignment, skill attachments), take no action on upgrade — safe no-op.
