...
- The `budget_cap_tokens` field is surfaced in the API response but the column name conflict (ISSUE #1)
  does not affect the API shape — the JSON key is `budget_cap_tokens` regardless of the DB column name resolution.
