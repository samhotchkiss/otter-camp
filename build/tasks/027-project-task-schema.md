...
- ISSUE #7 (AMBIGUOUS): `merge_queue_entry.archived_at` is set when the **deploy including this entry completes**, NOT when the merge operation itself completes. Doc 03a is authoritative. The `merged_at` column records merge completion; `archived_at` records deploy completion. Both are nullable; a merged-but-not-deployed entry has `merged_at` set and `archived_at` null.
- ISSUE #3 (GAP): `inbox_item.item_type = 'browser_handoff'` is included in the check constraint per doc 03. The full `browser_handoff` table and the inbox item creation flow for browser handoffs are defined in task 059. This task only reserves the enum value.
