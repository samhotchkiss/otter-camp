...
> ✅ ISSUE #9 (RESOLVED): `memory_source.session_id` is a permanent soft reference (no SQL FK, by design). Memories outlive sessions. The retrieval pipeline does not query `memory_source` directly. Application-layer integrity only — use LEFT JOIN where needed.

> ✅ ISSUE #10 (RESOLVED): `memory_source.source_id` semantics per source_type: `chat_message`→chat_message.id, `event`→domain_event.id, `file`→null (no DB record), `import`→memory_import.id, `explicit`→null. No DB FK for any type — application-layer enforcement. Doc 06 design notes updated.
