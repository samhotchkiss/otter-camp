...
> ⚠️ ISSUE #9 (AMBIGUOUS): `memory_source` table (which records the originating session and source type for each memory) is not built until L4. The `GET /v1/memory/items/:id` response currently cannot include source provenance. Add a `"sources": null` field to the response with a comment in the handler: `// TODO: populate sources from memory_source table after task 075 (L4)`. This way the API contract is stable and the field can be populated later without breaking clients.
