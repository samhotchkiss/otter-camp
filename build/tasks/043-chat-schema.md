...
> ⚠️ ISSUE #9 (GAP): `memory_source.session_id` is a soft reference — no SQL FK. Records survive session deletion. Application-layer must handle the case where `session_id` points to a deleted session (return the `session_id` value as-is; do not attempt to join to `chat_session` without a LEFT JOIN guard). Do not add a FK constraint until Sam resolves whether ON DELETE behavior should be SET NULL or CASCADE.
