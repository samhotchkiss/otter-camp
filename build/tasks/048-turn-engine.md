...
> ✅ ISSUE #18 (RESOLVED): Turn-loop model calls set `run_attempt_id=NULL` and `turn_id=chat_turn.id`. `run_attempt_id` is only set for model calls within a control plane run attempt (worker-domain). This is the correct and final behavior.
