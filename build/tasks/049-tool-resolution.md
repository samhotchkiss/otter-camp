...
> ⚠️ ISSUE #25 (GAP): `flow_node.tool_domains` is defined in doc 20 but not yet present in the `flow_node` DDL (task 017). Stage 3 of the pipeline depends on this column. If the migration for `flow_node.tool_domains` has not been applied (ISSUE #16 resolution pending), Stage 3 must gracefully degrade: attempt to read `tool_domains`, catch a missing-column error or null value, and skip deprioritization. Do not finalize Stage 3 implementation until Sam resolves ISSUE #16 / ISSUE #25.
