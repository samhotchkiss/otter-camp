...
> ⚠️ ISSUE #7 (AMBIGUOUS): `merge_queue_entry.archived_at` trigger is ambiguous between doc 03 (merge completion) and doc 03a (deploy completion). This task implements the doc 03a authoritative rule: `archived_at` is set when the deploy including the entry completes, not on merge completion. `MergeWorker` does NOT set `archived_at`.
