...
Test setup: calls `POST /v1/test/reset` and `ottercamp bootstrap` before the scenario.
...
- [ ] Final `GET /v1/health` returns `healthy` after the full workflow
...
possible. Each scenario calls `POST /v1/test/reset` first, so they are independent. The
