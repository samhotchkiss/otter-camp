...
- Task work, review, reject-loop, and retry transcripts belong on per-execution async node sessions linked from `flow_node_execution.session_id`, not on the sync discussion session. Project-scoped sessions are for PM oversight/triage, not for storing task execution history or serving as the execution container for task wakeups.
- Task dispatch must target the correct execution session for the current `flow_node_execution`. If a stale run points at a project-scoped PM session or reaches scheduling with no execution session bound yet, the runtime repairs onto the canonical node session for that execution before continuing.
- If retries accidentally create duplicate async sessions for the same `flow_node_execution`, the runtime deterministically keeps one canonical node session for that execution and closes or ignores the blank duplicates. This rule applies per execution, not across the whole task.
...
- **Project session**: the project manager assigned to that project. Fresh bootstrap is the exception: before a PM exists, the first project turn routes to Frank and then hands off to Lori until staffing/bootstrap setup persists the PM assignment.
...
- **Multi-agent coordination**: each scope has a default responder (Frank for org, PM for steady-state project sessions, PM (or task lead) for task). Fresh project bootstrap is the exception: before a PM exists, the project session starts with Frank and hands off to Lori for staffing/setup. Other present agents listen and can interject via a lightweight eval pass when they have something urgent to contribute. No round-robin, no free-for-all.
