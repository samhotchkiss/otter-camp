...
This uses the normal flow system and normal scheduler rules. The bootstrap task's `blocks_scope = 'all'` gate remains the governance checkpoint for bootstrap completion and downstream scope release.
The gate does not open merely because the project session persisted assignments or draft tasks. The canonical bootstrap child tasks must still be complete, Frank sign-off must be recorded, and the parent bootstrap gate must verify those child outputs together before bootstrap itself reaches `done`. Until that gate opens, first-wave project tasks may be persisted as planned `draft` work but must not be promoted into runnable execution or claimable jobs.
...
- If planning metadata has already classified a task as an orchestration parent, the runtime must reject any direct `queued` or `in_progress` promotion until bounded child tasks actually exist. This guard is driven from persisted metadata/planning state, not from prompt wording or title heuristics.
...
- When a slot opens, the scheduler picks the highest-priority eligible task and first ensures the task has concrete flow execution state for its current/start node. Only after that state exists does it transition the task into `in_progress`, resolve the actor, resolve or create the canonical execution session for that flow node, and start a run. Project-scoped PM sessions may coordinate the work, but they are never the execution container for that task's kickoff or run history.
- Queue wakeups are single-owner and idempotent. Duplicate task-status or flow-advanced events must coalesce onto the existing live/deferred wakeup instead of starting overlapping runs, and a stale runtime owner must be retired or promoted through the explicit recovery/deferred-wakeup path before a new owner is allowed to execute that same task/node.
- The active `flow_node_execution` is the authoritative runtime owner for a task lane. The bound execution session, current live turn, active run, and any resumable checkpoint belong to that execution boundary rather than being inferred independently from the broader task/session surface.
- `work_status` is an operator-facing projection of flow/runtime truth, not an independent second controller. The product must not allow `draft`/`queued`/`in_progress`/`review`/`blocked`/`done` to drift away from the active flow-node execution, the current node type, or the current human/runtime gating condition.
- Queue-driven status promotion must be compare-and-swap, not "best effort". A stale queued-event consumer may only do `queued -> in_progress` if the task is still actually `queued` when that transition is attempted; once a newer state like `blocked`, `review`, or `cancelled` has landed, the old queued wakeup must fail closed as stale churn instead of reopening the task.
- Task-id based operator/service commands use the same optimistic-concurrency protection, but they may refetch and reapply once on `repo: conflict` when they did not ask for an explicit compare-and-swap guard. This keeps manual/system commands like `MarkBlocked` resilient to adjacent metadata churn without reopening stale queue transitions. Any command that provides an expected-from status still fails closed on mismatch.
- Project pause applies to deferred wakeups as well as fresh queue pickup. If a task's next legal owner is already deferred when the project is paused, the current owner may still release cleanly, but the deferred wakeup must remain held in `created`/resumable state until `project.resumed` explicitly re-promotes it. A paused project must not auto-start that follow-on run just because turn release happened after the pause.
- Flow-owned transitions are authoritative. When `flow.advanced` or `flow.rejected` moves a task onto a new node and also emits the paired `task.status_changed` event, that status-change payload must carry flow-transition metadata (`transition_source`, `flow_event_type`, `to_flow_execution_id`). The queue processor uses that metadata to suppress the redundant generic `flow_current` wakeup so the new node is resumed exactly once through the flow-transition path.
- Flow-owned transitions must also be atomic at the persistence boundary. Completing/rejecting the current execution, moving `current_flow_node_id`, creating the next execution, applying the paired task-status change, and recording/publishing the flow event must commit or roll back together. Late event-publish failures are not allowed to leave behind half-advanced runtime state, and native tools must not paper over that by writing rollback updates directly to `project_task`.
- Execution sessions are repairable, but only through the canonical node-session path. If a flow execution reaches the queue with a missing `flow_node_execution.session_id`, the scheduler/control plane must recover it via the chat service's canonical "get or create node session" operation before creating the wakeup run, rather than starting a run with a nil session or minting an ad hoc replacement.
- Execution-session identity is per `flow_node_execution`, not per task. Advancing or rejecting onto a new node/execution must not silently reuse the prior task async session just because the scope is the same project task; otherwise review/reject path ownership collapses onto the wrong live session.
- Each `flow_node_execution` closes against a single canonical commit on the task branch. Work nodes may create temporary commits during execution, but the runtime produces one final canonical commit for the execution before closing it. Review approvals always close with an empty canonical commit. Review rejections close with a canonical rejection commit; when the reviewer records inline repo feedback, that feedback must use CriticMarkup in review-scoped markdown artifacts. A flow-node execution is not allowed to leave `active` without a canonical commit hash.
- Task writes themselves must also fail closed on stale snapshots. Any status-bearing `project_task` update must use optimistic concurrency (or an equivalent row-locking guarantee) so an old queued/in-progress copy cannot overwrite a newer blocked/review/cancelled state after the fact.
...
Recovery itself must resume from a structured checkpoint tied to the active `flow_node_execution`. The system must not depend on broad historical draft inference across old chat messages, summaries, or workspace scans to guess the next deliverable body or target path after the failing turn has already identified the resumable artifact.
...
- That PM triage session is not the task's work log. The task's execution/review transcript stays on the canonical execution session for the active `flow_node_execution` so the operator sees the real task history in task detail. If duplicate blank sessions exist for that execution, the runtime collapses them back to the canonical execution session before dispatching more work.
...
- **Tasks are flat in storage, not in runtime intent.** There is no dedicated task-to-task `parent_task_id` column, but decomposition metadata can still define parent/child orchestration relationships. Parent tasks are orchestration/integration-only once executable child tasks exist.
  When every executable child has reached a terminal state and the parent already has enough persisted structure to prove the integration outcome, the parent may auto-settle through the canonical task transition service instead of lingering forever as a manual clean-up shell. For decomposition parents, that means the runtime can synthesize missing child-verification, passed integration-check, and satisfied outcome-assessment metadata from the completed child set before transitioning the parent to `done`. This is a task-service invariant, not a one-off operator repair.
...
- **Every new project starts with a canonical bootstrap task tree.** Task 1 is a `blocks_scope = 'all'` governance gate, and project creation also seeds bounded bootstrap setup child tasks for repo/environment binding, staffing, decomposition, task-shape validation, flow attachment/validation, first-wave selection, and Frank sign-off. Those setup tasks remain orchestration-only, and any bounded first-wave project-task children they emit remain planned `draft` work until the parent bootstrap gate verifies the setup outputs together and records Frank's sign-off. Bootstrap task creation must go through the canonical task service, not direct repo writes, and any starter-agent assignees used by that bootstrap tree must be persisted as active project assignments before the bootstrap tasks are created. The governance gate is fail-closed for execution, not for bootstrap planning: while that root bootstrap gate is outstanding, the project session is still allowed to persist the bounded non-bootstrap planning task tree that bootstrap validation requires.
  Bootstrap planning shells follow the same cleanup principle. A `Bootstrap:` planning task with an enforced playbook contract and recorded planning artifacts should not remain permanently `draft` once the system can prove its required artifacts exist. The canonical task transition service may auto-complete those bootstrap planning tasks by synthesizing minimal `planning.artifact_evidence` from the recorded planning artifacts, then validating the planning contract and settling the task to `done`. This keeps finished projects from ending with stale bootstrap-planning residue while still using the normal validator path.
  The kickoff/bootstrap conversation in the canonical project session is an active bounded workflow, not a passive chat plan. After Frank hands off to Lori, the system may enqueue automatic follow-on bootstrap turns until persisted setup records exist or the session records an explicit bootstrap failure instead of silently idling. That handoff is a product invariant, not just a prompt suggestion: if org-side project creation succeeds and a canonical async project session exists but Frank failed to send the kickoff message, the turn engine must synthesize the handoff from the originating request and queue Lori onto the project session instead of leaving the project stranded with an empty chat.
  ...
  If a bootstrap follow-on turn still has zero persisted assignments/tasks/flow templates and a provider invocation stays `in_flight` past the bootstrap watchdog bound (default: 90 seconds), that turn must fail explicitly, the claimed `agent_turn` job must settle instead of remaining claimed forever, and the same project/session must be retryable without manual DB cleanup.
  Project creation and kickoff task creation must bind the project to its canonical workspace repo path before Lori emits the persisted task tree, so the same project session can create parent and child tasks without a separate operator repo-setup step. That binding is not just a database row: the canonical workspace path must exist on disk as an initialized git repo on `main`, or repo-backed planning/task tools must fail the binding contract instead of pretending the project is ready.
  The project session metadata carries a machine-checkable `project_bootstrap` contract (`active`, `completed`, or `failed`) with explicit persisted checkpoints (`project_created`, `staffing_persisted`, `task_tree_persisted`, `flow_templates_persisted`, `first_wave_selected`, `first_wave_executions_created`, `first_wave_jobs_claimed`) plus validator fields (`validation_status`, `validation_failure_class`, `validation_failure_reason`, `first_wave_task_count`, `first_wave_promoted_count`, `first_wave_execution_count`, `first_wave_job_count`). The same runtime snapshot is mirrored into `project.settings.project_bootstrap` so project-level surfaces can read the current phase and last successful checkpoint without replaying session chatter. The contract also persists `current_phase`, `last_successful_checkpoint`, per-checkpoint status/timestamps, and structured `validation_findings`. Findings must classify at least execution-shape violations, bounded task-size/decomposition violations, and provider/API failures that blocked progress before bootstrap crossed into runnable first-wave work. `phase` / `last_checkpoint` remain available in session metadata for runtime recovery paths, and `failure_phase` pins the checkpoint where an automatic archive or pause decision was made. The bootstrap invariant is binary: persisted setup must either advance through `first_wave_jobs_claimed` with real execution artifacts, or fail cleanly at `first_wave_selected` / `first_wave_executions_created` before the project keeps running as an active bootstrap. That fail-closed rule still applies on hardened rebuild paths even if a buggy setup pass omitted the canonical bootstrap gate task entirely; the reduced samblog-rebuild shape with 4 active assignments, 2 runnable flow templates, and 5 top-level first-wave tasks must either materialize real `flow_node_execution` rows plus runnable task-session jobs immediately or fail/archive before the project can remain active.
  Fresh bootstrap also persists a canonical restart bundle on the project before any automatic archive/restart loop runs. That bundle contains the originating operator brief, explicit operator constraints, the repo/environment/credential bindings needed to replay bootstrap cleanly, a bounded retry budget, the consumed retry count, and structured failure history carried forward across each fresh restart.
  Bootstrap only reaches `completed` when active project assignments exist, any compound kickoff parent workstreams have been split into bounded child tasks, the first executable wave points at runnable flow templates, those first-wave tasks still have a persisted repo/workspace binding to run in, the canonical bootstrap setup child tasks have reached `done`, Frank sign-off has been recorded, the parent bootstrap gate has persisted child verification plus a passed integration/outcome check, and the system has then promoted at least one first-wave child out of `draft` into actual execution artifacts through the `first_wave_jobs_claimed` checkpoint. Persisted setup may identify the intended first wave before bootstrap completion, but the governance gate stays fail-closed for execution: while bootstrap is still outstanding, those first-wave tasks must remain non-runnable and no task-session jobs may be claimable. The project session must still be able to persist the non-bootstrap planning task tree while that gate is open, otherwise bootstrap deadlocks itself. First-wave inference is not allowed to default to "every runnable leaf task": dependency-blocked tasks stay out of the first wave, and tasks explicitly marked as later work (for example `NEXT WAVE`, `BOOTSTRAP COMPLETE`, or a numbered `Wave 2+` title/metadata hint) must remain `draft` until the project session advances them in a later wave. If setup has already persisted staffing plus the bootstrap-only task/template shape but still has not emitted any executable non-bootstrap project tasks, that is also an explicit bootstrap failure: the project must fail at `first_wave_selected` instead of idling as an "active" bootstrap with only governance/setup records. The automatic failure record for the zero-execution path must pin `failure_phase` / `last_checkpoint` to `first_wave_executions_created`, not merely `first_wave_selected`, because the defect is failure to materialize child execution after the gate opens. Bootstrap setup child tasks themselves stay bounded and orchestration-only: if a setup task grows large enough to need decomposition, or if executable project tasks are hidden underneath a setup task instead of being emitted as normal project tasks, the validator must fail bootstrap. Lightweight setup subtasks are allowed only as internal checkpoints and do not count as the real executable task tree. Canonical bootstrap setup completion is persisted through the dedicated `bootstrap.setup.persist` tool rather than by pretending those checklist steps are ordinary worker execution tasks. The parent-to-child handoff is strict: once bootstrap persists orchestration parent tasks, any follow-on `task.create` call for executable children must already describe a bounded child slice. If that child creation attempt is rejected by the bounded-parent guard, bootstrap must fail and archive on that same turn instead of falling back into more project-session planning chatter. Playbook-aware bootstrap updates must also be able to persist planning artifacts on already-created tasks: if a task update carries planning evidence but the task is missing `metadata.planning`, the runtime must hydrate that planning state from the current task title/description before enforcing the artifact contract instead of dead-ending bootstrap on a missing-state error. That final checkpoint requires runnable first-wave `agent_turn` jobs for task-scoped async sessions, not merely chat messages that describe the intended kickoff. The runtime wiring is part of that contract: the live worker turn engine and task-queue processor must enforce the bootstrap gate so persisted first-wave tasks remain in planned state until sign-off and verification complete, then materialize `flow_node_execution` rows and queued task-session jobs. Broad parent-only kickoff output, parent tasks entering execution while executable children already exist, non-runnable first-wave flows, provider/API bootstrap failures, or a task tree that exists only as bootstrap gate/setup draft records without any emitted executable project tasks or repo-bound runnable workspace are explicit bootstrap failures, not "good enough for now" plans. Once persisted setup is sufficient to prove that first-wave execution still has not materialized after the gate should have opened, any raced project-scoped continuation turn must fail in bootstrap preflight before the model can append more parent planning chatter or write additional planning artifacts, and the project must auto-archive on that same failure path rather than continuing parent-session churn.
  Automatic failure handling is phase-aware. Product/bootstrap defects before `first_wave_jobs_claimed` auto-archive the project because no durable task execution has started yet. Once `first_wave_jobs_claimed` has been reached, later execution/runtime failures auto-pause the project instead so existing work/artifacts are preserved.
  When a bootstrap-product failure auto-archives before `first_wave_jobs_claimed`, the clean restart path must create a fresh live project plus fresh project-scoped bootstrap session from that canonical restart bundle. The failed project/session/task tree remain archived for audit, but their chatter, partial task tree, stale wakeups, and stale runtime ownership are not injected back into the live restart context. Restart bootstrap follows the same fail-closed invariant as fresh kickoff: if the fresh restart only recreates the canonical scaffold and still does not emit staffed executable work or runnable execution, that restart must archive on that same turn instead of remaining active as a half-born project. Automatic bootstrap restarts are bounded (default: two retries beyond the initial run). Once that retry budget is exhausted, the system stops creating fresh projects and escalates with a structured failure report instead of looping forever.
  Provider/API failures are tracked as a separate `provider_api` failure category. They do not get mislabeled as bootstrap-product defects. Auth failures pause immediately because a human/operator fix is required. Rate-limit and transient provider failures are restartable while bootstrap has not yet persisted setup (`setup_persisted=false`), so they archive-and-restart through the bounded bootstrap retry path instead of freezing a half-born project. Once bootstrap setup has persisted or real execution has started, those same provider failures pause the project so existing work is preserved.
  Every automatic archive/pause also writes a structured project-level failure record (`automatic_failure`) with exact fields `action`, `source`, `failure_category`, `failure_class`, `failure_phase`, `last_checkpoint`, `last_successful_checkpoint`, `failure_reason`, `setup_persisted`, `retry_budget`, `retry_attempt_count`, `failure_history`, and `recorded_at`. `source` distinguishes failures emitted by `project_bootstrap` from later `execution_runtime` pauses, so runtime, operators, and UI clients can tell "fresh bootstrap should be abandoned" apart from "real execution exists; preserve it and investigate." That includes supervisor-detected impossible live execution state: when a flow-backed task loses both runtime ownership and active flow execution yet still claims `in_progress`, the system blocks that task and pauses the project under `execution_runtime` instead of surfacing only a vague stalled project. On exhausted bootstrap retries, that same record is the authoritative escalation payload.
...
- **Dependencies can be removed.** PM can unlink a dependency at any time. If a blocked task has no remaining unresolved dependencies, it re-enters `queued` and restarts through the scheduler.
