...
- **Every new project starts with a bootstrap gate task.** Task 1 is a `blocks_scope = 'all'` governance flow: Lori does staffing/decomposition work, Frank reviews it, and an explicit completion/merge node closes the gate before other tasks can start.
  ...
  The project session metadata carries a machine-checkable `project_bootstrap` contract (`active`, `completed`, or `failed`) with persisted-count progress plus explicit ordered checkpoints: `project_created`, `staffing_persisted`, `task_tree_persisted`, `flow_templates_persisted`, `first_wave_selected`, `first_wave_executions_created`, and `first_wave_jobs_claimed`. The same runtime snapshot is mirrored into `project.settings.project_bootstrap` so project-level surfaces can read the current phase and last successful checkpoint without replaying session chatter.
  The bootstrap contract also persists `current_phase`, `last_successful_checkpoint`, per-checkpoint status/timestamps, and structured `validation_findings`. Findings must classify at least execution-shape violations, bounded task-size/decomposition violations, and provider/API failures that blocked progress before bootstrap crossed into runnable first-wave work.
  Bootstrap only reaches `completed` when active project assignments exist, any compound kickoff parent workstreams have been split into bounded child tasks, the first executable wave points at runnable flow templates, those first-wave tasks still have a persisted repo/workspace binding to run in, and the system has promoted at least one first-wave child out of `draft` into actual execution artifacts through the `first_wave_jobs_claimed` checkpoint. That final checkpoint requires runnable first-wave `agent_turn` jobs for task-scoped async sessions, not merely chat messages that describe the intended kickoff. Broad parent-only kickoff output, non-runnable first-wave flows, provider/API bootstrap failures, or a task tree that exists only as draft records without repo-bound runnable workspace are explicit bootstrap failures, not "good enough for now" plans.
