...
- If bootstrap persists a wave- or phase-based parent-plus-child task tree, the runtime must immediately promote the executable child slice into real flow execution. Persisting assignments, one or more runnable flow templates, and a draft-only task tree is not a valid active bootstrap state.
