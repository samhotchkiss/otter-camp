...
- **Every new project starts with a bootstrap gate task.** Task 1 is a `blocks_scope = 'all'` governance flow: Lori does staffing/decomposition work, Frank reviews it, and an explicit completion/merge node closes the gate before other tasks can start.
  ...
  Bootstrap only reaches `completed` when active project assignments exist, any compound kickoff parent workstreams have been split into bounded child tasks, and the first executable wave points at runnable flow templates. Broad parent-only kickoff output or non-runnable first-wave flows are explicit bootstrap failures, not "good enough for now" plans.
