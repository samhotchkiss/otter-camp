...
- **Every new project starts with a bootstrap gate task.** Task 1 is a `blocks_scope = 'all'` governance flow: Lori does staffing/decomposition work, Frank reviews it, and an explicit completion/merge node closes the gate before other tasks can start.
  ...
  The project session metadata carries a machine-checkable `project_bootstrap` contract (`active`, `completed`, or `failed`) with persisted-count progress plus validator fields (`validation_status`, `validation_failure_class`, `validation_failure_reason`, `first_wave_task_count`) so operators and UI clients can see whether staffing/task/flow setup actually materialized.
  Bootstrap only reaches `completed` when active project assignments exist, any compound kickoff parent workstreams have been split into bounded child tasks, the first executable wave points at runnable flow templates, and those first-wave tasks still have a persisted repo/workspace binding to run in. Broad parent-only kickoff output, non-runnable first-wave flows, or a task tree that exists only as draft records without repo-bound runnable workspace are explicit bootstrap failures, not "good enough for now" plans.
