...
The gate does not open merely because the project session persisted assignments or draft tasks. The canonical bootstrap child tasks must be complete, Frank sign-off must be recorded, and the parent bootstrap gate must verify those child outputs together before first-wave execution may start.
...
- **Every new project starts with a canonical bootstrap task tree.** Task 1 is a `blocks_scope = 'all'` governance gate, and project creation also seeds bounded bootstrap setup child tasks for repo/environment binding, staffing, decomposition, task-shape validation, flow attachment/validation, first-wave selection, and Frank sign-off before other tasks can start.
