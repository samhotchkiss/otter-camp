...
The gate does not open merely because the project session persisted assignments or draft tasks. The canonical bootstrap child tasks must still be complete, Frank sign-off must be recorded, and the parent bootstrap gate must verify those child outputs together before bootstrap itself reaches `done`. But once persisted setup already includes runnable bounded child work, the runtime must immediately promote that first-wave child slice into execution instead of leaving the tree in `draft` while the gate stays open.
...
- **Every new project starts with a canonical bootstrap task tree.** Task 1 is a `blocks_scope = 'all'` governance gate, and project creation also seeds bounded bootstrap setup child tasks for repo/environment binding, staffing, decomposition, task-shape validation, flow attachment/validation, first-wave selection, and Frank sign-off. Those setup tasks remain orchestration-only, but once persisted setup has also emitted runnable bounded project-task children, the runtime must promote that first executable child wave immediately instead of waiting for the parent bootstrap gate to finish.
