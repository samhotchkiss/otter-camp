...
Project creation now seeds a canonical bootstrap task tree — a root governance gate plus seven bounded setup child tasks covering repo/environment binding, staffing, decomposition, task-shape validation, flow attachment/validation, first-wave selection, and Frank sign-off. This ensures setup state is task-backed rather than only session-transcript-backed, giving the system explicit restart points and reviewable setup artifacts.
...
Bootstrap completion is validator-driven, not prompt-wording-driven: Lori's kickoff work is incomplete unless active project assignments are persisted, broad parent workstreams are decomposed into bounded child tasks, and the leaf first-wave tasks point at runnable flow structures inside a persisted repo/workspace binding for that project. Enumerated batch titles such as "generate 20 ideas across all pillars" count as broad compound work even if the description is short; the planner must decompose them before queueing rather than letting bootstrap discover the problem later. The bootstrap setup tasks themselves must also stay bounded and orchestration-only: they may use lightweight subtasks as internal checkpoints, but they cannot absorb deliverable work or hide executable project tasks underneath a setup task. If Lori persists only parent workstreams, oversized setup tasks, leaf tasks with non-executable flows, or a task tree whose project repo binding is still missing, the project bootstrap contract must stay failed/blocked instead of quietly passing review.
