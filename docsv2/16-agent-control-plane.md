...
- The supported recovery action is an explicit task resume. Resume clears the persisted validation guard, emits an auditable recovery/status event, re-queues the task, and allows the next wakeup to open a fresh live turn on the task's canonical async session. That kickoff must stay marked as a recovery turn even when it is emitted through the normal task-queue wakeup path instead of the supervisor literal prompt.
