...
- Terminal `flow.advance` must also be atomic. Completing the current execution, transitioning the task to `done`, clearing `current_flow_node_id`, recording the task event, and publishing the domain event happen in one transaction so a late publish failure cannot leave the task partially completed and force native-tool rollback writes.
