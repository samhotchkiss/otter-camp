...
- Bootstrap failure disposition is not one-size-fits-all. Pre-execution bootstrap/product defects auto-archive, but provider/API outages or any failure after `first_wave_jobs_claimed` must pause the project and persist the structured `automatic_failure` record instead of discarding already-started work.
