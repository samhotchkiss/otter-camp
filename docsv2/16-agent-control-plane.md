...
- Declaring an active owner stale must clear the active-owner pointer before failing that stale run. Otherwise the transient failure path can auto-promote the deferred wakeup too early and the next explicit wakeup will incorrectly start a brand-new run instead of promoting the already-deferred one.
- The same promotion must happen when the blocking active run fails transiently or times out. A resumable task cannot be left `in_progress` with only deferred wakeups and no runnable turn after the active owner has already been released.
- The same promotion must happen when an active work run completes and hands control to a deferred review/merge wakeup on the same task scope. Completion must release the current owner so the downstream node-scoped wakeup can start immediately instead of remaining deferred behind a finished run.
- If a deferred downstream wakeup is promoted by generic owner release before the queue processor sees the terminal turn event, the queue processor must still append the missing kickoff message into the current node-scoped session. Promotion without kickoff is not a valid runnable state.
- If that promoted downstream run only resolves its node-scoped session at dispatch time, the queue processor must bind the run to that session before appending kickoff. Supervisor recovery and runtime-state session metadata must never keep a nil or stale session once a node-scoped session exists.
...
- A blocked task that carries a durable recovery checkpoint remains `resumable` in `runtime_state`. Its active run fails, but the runtime contract must not be promoted to `terminal/permanent` unless the blocked condition is truly non-resumable.
- When a task already carries a durable recovery checkpoint, work re-entry triggered by `flow.rejected` must be treated as a recovery turn, not a fresh exploratory kickoff. The next turn must re-root from the checkpointed target/artifact state so rejected work does not drop back into broad repo-reading or generic retry chatter.
...
- A flow-backed wakeup that already has a node/work session recorded on its `flow_node_execution` must dispatch into that existing session instead of blindly creating a replacement canonical task session first. Session rebinding is allowed to repair stale run/session associations, but it must not discard a valid execution session that already exists for the node.
- Task-scoped `runtime_state` must always reflect the identity of the newest active or promoted run. When a task advances from one flow-node execution/session pair to another, the runtime contract must overwrite stale `session_id`, `flow_node_id`, and `flow_node_execution_id` values instead of preserving older review/work bindings from prior runs.
- Task-bound wakeups are only dispatchable while the task is still in a runnable work state. Once the task leaves `queued`/`in_progress` work-node execution and moves into `review`, `done`, or another non-work state, stale supervisor recovery messages and pending `agent_turn` wakeups for that task session must be cancelled instead of opening another task turn.
...
- Flow-transition wakeups must be keyed to the concrete transition event, not just the destination `flow_node_execution_id`, so a failed review/work handoff can be retried on a later domain event without re-promoting the stale failed run.
- When a flow-transition kickoff targets a review node, dispatch must honor the review-node contract even if the task row still temporarily carries stale `blocked` status from the prior work execution. Stale task-level validation guards may only suppress turns in the same session that produced them.
- Review-node flow-transition kickoffs must be framed as bounded review work, not fresh task execution. The kickoff message must tell the reviewer to inspect current outputs, decide approve vs reject, and avoid rewriting the deliverable inside the review turn.
...
- The supported recovery action is an explicit task resume. `POST /v1/tasks/:id/resume` is allowed when the stored blocker class is `deterministic_validation_loop` or `durable_recovery_checkpoint`; if blocked-task metadata has lost the checkpoint but the recorded blocker reason, recovery kickoff/session metadata, recent recovery file-output history, or on-disk target/artifact files still deterministically identify the current durable checkpoint, the resume surface must rebuild that metadata before deciding the task is non-resumable. Recovery checkpoint reconstruction must not promote arbitrary context reads into the target path; only write-side evidence, recovery artifacts, or the task's explicit deliverable path are authoritative for the resumed target file. Other blocked states return a machine-checkable blocker-class error instead of silently re-queueing work.
- Validation-loop resume clears the persisted validation guard, emits an auditable recovery/status event, re-queues the task, and allows the next wakeup to open a fresh live turn on the task's canonical async session. Durable-checkpoint resume preserves the checkpoint metadata so the next wakeup remains a recovery turn and seeds itself from the target draft, recovery artifact, and persisted failure reason rather than a generic continuation prompt; rebuilt checkpoints from verified on-disk state follow the same path. If a blocked task still carries a stale raw validation guard after durable file-output checkpoint metadata has been rebuilt or persisted, the resume surface must prefer the durable checkpoint contract and clear the guard in the same transition instead of downgrading the task back to a generic validation-loop retry. If that persisted failure reason already says the last recovery draft was non-substantive, the resumed turn must harden its contract: rejected placeholder text is invalid context, any durable draft excerpts that match the rejected pattern must be omitted from the prompt, and the next attempt must begin with substantive file-body content rather than another narration of intent. That hardened path must activate as soon as there is a rejected-draft checkpoint with no substantive durable target/artifact draft left on disk; it must not wait for a second live failure just because the rejected drafts were non-empty. The same hardened path also applies when a substantive target draft already exists on disk but the remaining recovery artifact/checkpoint draft was rejected as placeholder narration: the existing file becomes the revision baseline, the rejected artifact excerpt is omitted, and the resumed turn still enters write-only recovery mode instead of falling back to generic repo-reading. Once the checkpoint has reached repeated non-substantive-recovery status, that same hardened mode must continue even if a substantive target draft still exists on disk; the existing file becomes protected context, not permission to fall back into surgical tool edits. In that hardened state, the resumed turn must root prompt history at the recovery checkpoint instead of the original kickoff and run in write-only recovery mode so the model cannot drift back into exploratory repo-reading before it writes the target body. Write-only recovery means no tools are exposed to the model for that turn; the assistant must emit only the target file body as plain text and runtime performs the final `file.write` itself once the body is substantive.
- Flow-node execution sessions are keyed to `flow_node_execution_id`, not merely to the task. If a stale task/session binding points a work execution at a review session (or vice versa), runtime must repair that binding on the next active-execution load before dispatching another turn, and node-scoped session creation must bypass generic task-async canonical-session reuse.
...
If a `flow.rejected` wakeup returns a task to a work node while that task still has durable recovery checkpoint metadata, the kickoff must inherit recovery semantics. The task queue processor may still author the wakeup, but the turn engine must treat that message as recovery-rooted work so the checkpointed target file, artifact draft, and failure metadata remain authoritative.
