...
- The supported recovery action is an explicit task resume. `POST /v1/tasks/:id/resume` is allowed when the stored blocker class is `deterministic_validation_loop` or `durable_recovery_checkpoint`; if blocked-task metadata has lost the checkpoint but the recorded blocker reason plus on-disk `.ottercamp/recovery/...` and target files deterministically prove the current durable checkpoint, the resume surface must rebuild that metadata before deciding the task is non-resumable. Other blocked states return a machine-checkable blocker-class error instead of silently re-queueing work.
