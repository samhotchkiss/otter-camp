...
The validator is fail-closed against persisted state, not chat prose. `completed` requires active assignments, bounded leaf first-wave tasks, runnable flow templates for those leaf tasks, and the bootstrap handoff crossing the `first_wave_jobs_claimed` checkpoint. That checkpoint means runnable first-wave `agent_turn` jobs exist for the task-scoped async sessions; a kickoff chat message without a queued job does not satisfy it. Missing assignments, compound parent-only kickoff output, non-runnable first-wave flows, and provider/API bootstrap failures must surface as structured `validation_findings` plus `validation_failure_class` / `validation_failure_reason` on the same `project_bootstrap` contract so runtime consumers can act on the exact failure phase deterministically.
