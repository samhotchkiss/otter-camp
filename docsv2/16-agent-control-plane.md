...
The worker loop claims jobs conservatively. A single worker process may have a configured batch size for maintenance/testing APIs, but live execution must not pre-claim a large batch and then process it serially. In the runtime loop, each worker claims one runnable job at a time, executes it, settles it, and only then claims the next. This prevents long-running jobs such as summarization or model-bound turns from starving later `agent_turn` work by sitting in `claimed` state without execution.
...
- Project pause is an explicit gate on that promotion rule. If the project is paused when the current owner releases, the deferred wakeup is held in resumable state instead of being promoted immediately; `project.resumed` re-promotes and dispatches those held deferred wakeups before ordinary queued pickup resumes.
- That promotion rule applies even when the task's legal flow state changed while the previous owner was finishing. For example, a worker finishing a `work` node may defer a reviewer wakeup while the task is already `review`, and a reviewer rejecting back to a `work` node may defer the worker wakeup while the task is already back in `in_progress`. Turn completion must still promote the deferred canonical follow-on owner instead of dropping it because the task left the previous owner's status.
...
- Task-bound wakeups must resume on the execution session bound to the current `flow_node_execution`. A project-scoped session may coordinate about the task, but it is not the execution container for that task's run history, and the sync discussion session is not a substitute for execution lineage either.
- Wakeup dispatch canonicalizes the execution session before appending kickoff chatter. If an older run references a project-scoped PM session, a discussion session, or a missing/blank execution session, the control plane repairs onto the canonical node session for that `flow_node_execution` and closes or ignores duplicate blank sessions deterministically.
- Stale queue wakeups are not allowed to reinterpret state. When a queued-task consumer tries to promote work into `in_progress`, it must assert that the task is still `queued` at that moment; if a newer state such as `blocked`, `review`, or `cancelled` already landed, the wakeup is stale and must stop without reopening the task.
- Task-id based service commands use the same optimistic-concurrency guardrails, but when they did not request an explicit expected-from status they may refetch the latest task row and reapply once after `repo: conflict`. That keeps operator/runtime commands such as `MarkBlocked` resilient to adjacent queue metadata churn while preserving fail-closed compare-and-swap semantics for queue pickup and other expected-from transitions.
- A flow-backed task in `in_progress` cannot keep an active `flow_node_execution` unless that execution session also has a live task turn (`current_turn_id` bound to a pending/in-progress turn). If the turn disappears, the supervisor must either recreate one valid wakeup/turn pair on that same execution session or transition the execution out of `active`.
- Recovery-created or retry-created turns must reconcile `chat_session.current_turn_id` immediately. If a stale `pending` turn from another wakeup still exists, recovery cancels or replaces it so the session retains exactly one valid live turn before dispatch continues.
- If no recovery agent/session can be resolved, the supervisor marks the runtime contract `stranded`, abandons the active flow execution, and moves the task out of misleading `in_progress` so operators see the failure truthfully instead of a phantom live execution.
- If a flow-backed task still claims `in_progress` after both runtime ownership and active flow execution are gone, supervisor must fail closed and block it as impossible live state instead of leaving the project looking active forever. That is not resumable stranded execution; it is corrupted runtime truth.
...
- Policy-driven async review pauses use that same contract. A hard-stop or prepare-for-review checkpoint must advance onto the real review node/execution and then hold dispatch if policy says no owner should run yet; it must not leave the task in fake `review` or stranded `in_progress` with no runnable lineage.
- Human review actions use that same flow contract. Task endpoints and inbox actions must route approve/reject decisions through canonical flow transitions, not through direct task-status writes that skip `flow_node_execution` lineage.
- Before a restart-driven full end-to-end run, an automated restart-readiness smoke gate must pass. It is fail-closed and proves five invariants against the real runtime stack: fresh kickoff creates exactly one active project/session path; queued work reaches `in_progress` with a bound run and flow execution; a task can persist on-disk output and advance to `review`; a long-running content-migration task resumes cleanly across continuation turns without looping on continuation depth; and UI consumers reconcile project/task/worker state to the same control-plane truth.
...
Fresh project bootstrap uses that same canonical project session as the coordination surface. Project creation seeds a canonical bootstrap task tree up front: a root governance gate plus bounded setup child tasks for repo/environment binding, staffing, decomposition, task-shape validation, flow attachment/validation, first-wave selection, and Frank sign-off. The session metadata carries a machine-visible `project_bootstrap` contract with bounded states (`active`, `completed`, `failed`), persisted setup counts, ordered checkpoints, validator findings, and failure detail. At minimum the checkpoint sequence is `project_created` -> `staffing_persisted` -> `task_tree_persisted` -> `flow_templates_persisted` -> `first_wave_selected` -> `first_wave_executions_created` -> `first_wave_jobs_claimed`. The runtime also mirrors the same phase snapshot into `project.settings.project_bootstrap` so project-level state can report `current_phase` and `last_successful_checkpoint` without replaying the chat log. Automatic follow-on bootstrap turns may append system-generated continuation user messages to that same session until staffing/task/flow setup materializes or the session records an explicit bootstrap failure instead of silently idling. Once staffing/task/flow setup exists, the runtime must stop treating that session alone as sufficient to open execution: first-wave promotion remains blocked until the bootstrap child tasks are complete, Frank sign-off is recorded, and the parent bootstrap gate has persisted child verification plus a passed integration/outcome check. If prompt/context guardrails or bounded continuation depth exhaust before the required staffing/task/flow records exist, the runtime must mark `project_bootstrap.status='failed'` with machine-visible `failure_class` / `failure_reason` and stop automatic churn. If the bootstrap turn still has zero persisted setup artifacts and the active provider/model invocation remains `in_flight` past the bootstrap watchdog bound (default: 90 seconds), the runtime must fail that turn, release the claimed `agent_turn` job, and leave the session retryable instead of looking indefinitely active.
Before any automatic archive/restart path runs, the runtime persists a canonical bootstrap restart bundle on the project itself. That bundle captures the originating operator brief, explicit operator constraints, and the repo/environment/credential bindings required to replay bootstrap cleanly without consulting failed transcript sprawl.
  The same contract exposes phase-aware bootstrap checkpoints through `phase` / `last_checkpoint`, persists `failure_phase` when the runtime makes an automatic archive/pause decision, and carries structured `validation_findings` plus `validation_failure_class` / `validation_failure_reason`. The validator is fail-closed against persisted state, not chat prose. `completed` requires active assignments, bounded leaf first-wave tasks, runnable flow templates for those leaf tasks, completion of the canonical bootstrap child tasks, recorded Frank sign-off, parent bootstrap verification of those child outputs, and the bootstrap handoff crossing the `first_wave_jobs_claimed` checkpoint. First-wave selection must stay explicit and bounded at the control-plane layer: dependency-blocked tasks remain deferred, and tasks already marked as later waves in persisted title/metadata hints must not be auto-promoted just because they are otherwise runnable. Bootstrap setup child tasks are validated by that same bounded-scope policy: setup tasks may keep lightweight subtasks as internal checkpoints, but executable project tasks must still materialize as normal project tasks rather than hiding beneath a setup task. That checkpoint means runnable first-wave `agent_turn` jobs exist for the task-scoped async sessions; a kickoff chat message without a queued job does not satisfy it. Missing assignments, compound parent-only kickoff output, oversized setup tasks, a parent entering execution while executable children already exist, non-runnable first-wave flows, a first-wave task that still violates bounded-size decomposition, or provider/API bootstrap failures must surface on the same `project_bootstrap` contract so runtime consumers can act on the exact failure phase deterministically. Planning-artifact enforcement is part of that control-plane contract: when bootstrap updates an already-created task with playbook evidence, the runtime must hydrate missing `metadata.planning` from the task itself before applying artifact-contract checks, rather than letting a missing planning state block the setup-to-execution handoff. If persisted setup plus completed bootstrap child tasks still leave first-wave execution missing, any raced project-scoped continuation must be rejected in bootstrap preflight before the model emits more parent planning output or touches planning artifacts, and the same path must drive the automatic archive/pause decision immediately.
Automatic failure disposition is phase-aware and persists at the project level. Product/bootstrap defects before `first_wave_jobs_claimed` auto-archive the project because no durable task execution exists yet. Provider/API failures are classified separately as `provider_api` so they are not mislabeled as bootstrap-product defects. The action is class-sensitive: provider auth failures pause immediately because a human/operator fix is required; provider rate-limit and transient failures archive-and-restart while bootstrap has not yet persisted setup (`setup_persisted=false`), then pause once setup has persisted or real execution exists so existing work is preserved. Once `first_wave_jobs_claimed` has been reached, later task/runtime failures also pause the project so execution artifacts are not discarded. That same pause contract applies when supervisor finds a corrupted live execution state, such as a flow-backed task still claiming `in_progress` after it lost both runtime ownership and active flow execution; supervisor blocks the impossible task and pauses the project with an `execution_runtime` failure record instead of leaving the project merely "active but stalled." Each automatic archive/pause writes `project.settings.automatic_failure` with exact fields `action`, `source` (`project_bootstrap` or `execution_runtime`), `failure_category`, `failure_class`, `failure_phase`, `last_checkpoint`, `last_successful_checkpoint`, `failure_reason`, `setup_persisted`, `retry_budget`, `retry_attempt_count`, `failure_history`, and `recorded_at`.

Turn settlement is fail-closed with respect to invocation persistence. If a provider returns output but the runtime cannot persist `model_invocation` completion, the turn must fail and the worker job must not settle as successful. The control plane cannot leave a session with `chat_turn.status='in_progress'`, no live `agent_turn` job, and a lingering `model_invocation.status='in_flight'`; that is a runtime bug and must be surfaced as a failed turn instead of implicit success.
For bootstrap-product auto-archives, the follow-on restart does not resume the failed project session. It creates a fresh live project and fresh project-scoped bootstrap session from the persisted canonical bundle, while the failed project/session tree stays archived for audit only. That restart path is fail-closed under the same bootstrap contract: if the new project only recreates the canonical bootstrap scaffold and does not cross into staffed executable work or runnable execution, the runtime archives that restart immediately instead of leaving an active scaffold-only project behind.
...
- **Task-scoped**: `system.file.write` and `system.cli.execute` at task level operate inside the bound task's project workspace root. Task scope is the authorization and binding boundary, not a separate on-disk subtree. Files, manifests, scripts, and outputs created through file tools, git tools, or CLI are immediately shared across those surfaces for that task.
- **Task-scoped long-running migrations/imports**: when a task is clearly operating as a content migration/import, prompt-budget rollover is handled by writing a deterministic checkpoint into the bound task/project workspace and resuming from that checkpoint boundary. The control plane does not rely on replaying raw fetched content in later turns. If the checkpoint contains persisted scripts/artifacts but no migrated outputs yet, the resumed turn must prioritize emitting the first output file from that persisted state before generating more helper scaffolding.
...
- Repeated `cli.execute` validation failures are fingerprinted by the attempted command after `_raw` and legacy alias normalization, so recoverable serialization glitches do not turn valid recovery commands into deterministic `command_required` blocker loops.
- When repeated deterministic validation failures hit the blocker threshold, the control plane must persist the validation-guard fingerprint on the task, move the task to `blocked`, and suppress ordinary follow-up wakeups for that task until an explicit recovery action happens.
- Blocking a task for deterministic validation, bounded recovery, or stranded-execution reasons must always go through the canonical task/flow transition service. If that service is unavailable, runtime and supervisor paths must fail loudly and preserve the current persisted task state instead of writing `work_status='blocked'` directly as a degraded fallback.
- Bootstrap gate auto-complete follows the same rule. Even though it is a system-only exception to the normal `draft -> done` matrix, it must run through the canonical transition service with explicit audit payloads instead of hand-writing task rows/events in the turn engine.
- Orchestration-parent cleanup follows the same rule. When completed child work proves the parent integration outcome, the control plane may ask the canonical task transition service to auto-complete the parent, but it must not hand-write `done` onto the task row. The service is responsible for synthesizing any missing parent orchestration metadata and validating completion before the transition lands.
- Bootstrap planning cleanup follows the same rule. If a `Bootstrap:` planning task is still `draft` even though its recorded planning artifacts satisfy the enforced playbook contract, the control plane may trigger canonical auto-complete, but the task service must synthesize the missing `planning.artifact_evidence`, validate the artifact contract, and then persist the `done` transition itself.
- Pool-backed native task mutation tools follow the same rule. When `task.update` or queue-decomposition paths change task status in the live product, they must persist non-status edits first and then call the canonical task transition service instead of publishing synthetic `task.status_changed` events from the tool layer.
- Flow-owned runtime transitions must be atomic. Terminal `flow.advance`, non-terminal `flow.advance`, and `flow.rejected` all commit their execution-state changes, task-status transition, task event, and domain event in one transaction so a late publish failure cannot leave partial runtime state behind. Native tools must not try to compensate with direct task-row rollback writes; the flow/task services own correctness.
- Execution wakeups must fail closed on missing session lineage. If a flow execution reaches scheduling without `flow_node_execution.session_id`, the control plane must repair it through the canonical node-session service (`GetOrCreateNodeSession`) before it creates the wakeup run. It must not enqueue a run with a nil session and hope later layers infer one.
- That session lineage is per flow execution. The flow-session bridge must bind/reuse sessions by `flow_node_execution_id`, not by the broader task async scope, so reject/review follow-on work cannot inherit the previous node's live session by accident. If post-commit binding still misses transiently, the already-committed flow transition remains authoritative and queue-time repair closes the gap.
- The supported recovery action is an explicit task resume. `POST /v1/tasks/:id/resume` is allowed when the stored blocker class is `deterministic_validation_loop` or `durable_recovery_checkpoint`; if blocked-task metadata has lost the checkpoint but the recorded blocker reason, recovery kickoff/session metadata, recent recovery file-output history, or on-disk target/artifact files still deterministically identify the current durable checkpoint, the resume surface must rebuild that metadata before deciding the task is non-resumable. Other blocked states return a machine-checkable blocker-class error instead of silently re-queueing work.
- Validation-loop resume clears the persisted validation guard, emits an auditable recovery/status event, re-queues the task, and allows the next wakeup to open a fresh live turn on the task's canonical async session. Durable-checkpoint resume preserves the checkpoint metadata so the next wakeup remains a recovery turn and seeds itself from the target draft, recovery artifact, and persisted failure reason rather than a generic continuation prompt; rebuilt checkpoints from verified on-disk state follow the same path. If that persisted failure reason already says the last recovery draft was non-substantive, the resumed turn must harden its contract: rejected placeholder text is invalid context, any durable draft excerpts that match the rejected pattern must be omitted from the prompt, and the next attempt must begin with substantive file-body content rather than another narration of intent.
- On that resumed recovery turn, malformed `cli.execute` calls with no recoverable `command` are not allowed to consume the full blocker budget again. The runtime injects one exact correction message in-turn and grants one bounded retry; if the agent repeats the same empty-command call, the turn halts in a diagnosable recovery state and, unless a real follow-on wakeup is already queued, the task must move out of misleading `in_progress` into `blocked` with operator guidance for the next explicit resume. If recovery history still identifies the intended file target (for example from a prior checkpoint or recent `file.read`/`file.write` results), that halt must persist the same `.ottercamp/recovery/...` artifact/checkpoint contract used for file-output recovery instead of ending with generic `model_error` and no durable anchor.
- On that same resumed recovery turn, malformed `file.write` calls with a target `path` but no concrete `content` must prefer artifact-first recovery. If the turn already produced a substantive assistant draft, the runtime carries that draft into the write. Self-referential recovery chatter is not a substantive draft, and neither is shallow intent narration or placeholder preamble that only announces the plan to write the deliverable; prose about missing tool parameters, conversation history, swapping from `file.write` to `cli.execute`, or saying “Now I have everything I need. Let me write...” must never be written into the target file as if it were deliverable content, whether the runtime inferred that body from nearby assistant text or the model supplied the same placeholder directly in `file.write.content`. If that is the only available assistant text, the runtime must halt with the same durable recovery artifact/checkpoint instead of auto-writing the chatter or clobbering an existing target draft. If the persisted checkpoint already records a non-substantive-draft failure and the resumed turn produces another semantically equivalent placeholder, the runtime must reject it as a repeated non-substantive draft, persist that stronger blocker reason durably, and stop the turn in the same bounded recovery checkpoint path instead of degrading back into generic `model_error` churn. That hardened stop state is explicit and machine-checkable: the checkpoint/task-blocked payload must carry `recovery_blocker_class=repeated_non_substantive_recovery_checkpoint` plus target path, artifact path, the current failure reason, and preserved prior failure reasons so `/v1/tasks/:id/resume` and queued wakeups can continue from the bounded checkpoint contract. A recovered write is only a success if the target file is durably present on disk after dispatch; if dispatch returns without the file landing, the turn must halt immediately, persist the last write failure on the recovery checkpoint/artifact, and stop same-turn retries for that path. If the project still has a legacy org-nested workspace root from before the flattened workspace rollout, the same recovery write verification must accept the file from either compatible project root instead of falsely halting after a successful dispatch. If no draft exists yet, the runtime injects one bounded correction that demands concrete file content before another write. A second empty-content retry halts the turn with a truthful persisted recovery contract: if the runtime can write a recovery artifact it must persist `.ottercamp/recovery/...` before telling the next turn to resume from it, and it must also persist task metadata that points the next turn at that checkpoint. When both the canonical flat workspace root and a legacy org-nested root already exist for the same project, the recovery artifact must be mirrored into both locations before the runtime claims that checkpoint exists. If no artifact was written, the halt message must say so plainly instead of claiming one exists. When that checkpoint already exists on a later explicit resume, prompt assembly must seed the turn from the durable target-file draft first, then the persisted recovery artifact draft, then the checkpoint metadata/failure reason, so the resumed turn keeps a bounded on-disk recovery anchor instead of asking which task it is resuming. The preferred stop reason is `recovery_content_required`; if the deployed `chat_turn` constraint does not yet accept that value, runtime must fall back to `model_error` rather than crashing turn persistence, but the checkpoint/task blocker class above remains the authoritative operator-facing contract. If no real continuation is already queued, that halted recovery turn must also move the task out of false `in_progress` into `blocked` so operators see a truthful bounded stop state, and if continuation-depth guardrails later exhaust that same resume attempt they must preserve the same durable checkpoint instead of first overwriting the target with placeholder narration. Supervisor stranded-execution recovery must not auto-resume that same halted turn again until the task advances past the recorded checkpoint or is explicitly re-queued.
- If that resumed recovery turn never reaches useful work because prompt assembly or input guardrails exhaust the continuation-depth budget, the runtime must still end truthfully. It must halt the turn with explicit operator guidance, suppress silent auto-continuation for that failed recovery loop, and either keep the task `in_progress` only when a real queued follow-on wakeup already exists or move the task to `blocked` with guidance that narrows the next resume attempt. If a recovery checkpoint already exists, the runtime must carry that bounded recovery state forward deterministically instead of falling back to a generic continuation summary, and any queued-wakeup claim must refer to a distinct pending/claimed follow-on job rather than the worker job that is currently executing. If task metadata dropped the checkpoint but the live recovery session still names the target or artifact through recovery kickoff metadata or recent `file.write`/`file.read`/`cli.execute` tool results, the guardrail halt must re-persist a checkpoint from that session state before the task settles into `blocked`.
- If a task turn hits provider-auth failures (`401`/`403`) on one enabled model connection, the gateway must mark that connection unavailable for the current wakeup and try the next healthy enabled connection or fallback profile before giving up. If no healthy continuation path exists, the turn must halt in a truthful blocked state instead of looping through generic `model_error` retries or wakeups, and the tracking run/runtime must stop advertising that task as active until an operator fixes or disables the bad credential and explicitly resumes the task.
- The control plane must not silently clear deterministic validation blockers in the background, and operators must never need direct DB mutation to recover the task.
...
- For fresh project bootstrap setup turns, the same bounded prompt/continuation guardrails fail closed: when the setup workflow cannot materialize persisted project assignments, scoped tasks, and flow templates before the limit is exhausted, the runtime records explicit `project_bootstrap.failed` state with failure detail instead of leaving the project session looking merely active.
- For fresh project bootstrap setup turns, a single stuck provider call is also fail-closed. If the bootstrap watchdog fires while setup counts are still all zero, the turn fails explicitly, the claimed kickoff job settles, and operators can retry or rotate the session without manual cleanup.
- Fresh bootstrap planning stays allowed to persist normal non-bootstrap project tasks even while the canonical governance gate is still outstanding. The gate blocks execution promotion, not the planning-time task/tree writes that bootstrap validation itself depends on.
- Canonical bootstrap checklist completion is a first-class native tool contract: `bootstrap.setup.persist` marks the bounded setup steps as persisted from the project session once their outputs exist, instead of forcing agents to fake worker-task status transitions for setup bookkeeping.
- Bootstrap failure disposition is not one-size-fits-all. Pre-execution bootstrap/product defects auto-archive. Provider auth failures pause immediately. Provider transient/rate-limit failures before setup persists consume the bounded clean-restart budget instead of pausing a still-empty project. After setup persists, or after `first_wave_jobs_claimed`, provider/API outages pause the project and persist the structured `automatic_failure` record instead of discarding already-started work.
- Clean bootstrap restarts are bounded. The runtime carries forward canonical bootstrap input plus retry count and structured failure history across each fresh restart, but after the configured retry budget is exhausted it must stop auto-restarting and escalate with a machine-readable failure report (`failure_phase`, `failure_class`, `failure_reason`, `last_successful_checkpoint`, `setup_persisted`, and retry history) rather than looping indefinitely.
