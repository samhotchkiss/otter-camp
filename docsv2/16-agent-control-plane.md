...
- On that same resumed recovery turn, malformed `file.write` calls with a target `path` but no concrete `content` must prefer artifact-first recovery. If the turn already produced a substantive assistant draft, the runtime carries that draft into the write. If no draft exists yet, the runtime injects one bounded correction that demands concrete file content before another write. A second empty-content retry halts the turn with a persisted recovery artifact under `.ottercamp/recovery/...` and a valid persisted stop reason instead of re-entering the old deterministic blocker loop. The preferred stop reason is `recovery_content_required`; if the deployed `chat_turn` constraint does not yet accept that value, runtime must fall back to `model_error` rather than crashing turn persistence.
