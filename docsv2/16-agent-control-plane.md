...
- The supported recovery action is an explicit task resume. `POST /v1/tasks/:id/resume` is allowed when the stored blocker class is `deterministic_validation_loop` or `durable_recovery_checkpoint`; other blocked states return a machine-checkable blocker-class error instead of silently re-queueing work.
- Validation-loop resume clears the persisted validation guard, emits an auditable recovery/status event, re-queues the task, and allows the next wakeup to open a fresh live turn on the task's canonical async session. Durable-checkpoint resume preserves the checkpoint metadata so the next wakeup remains a recovery turn and seeds itself from the target draft, recovery artifact, and persisted failure reason rather than a generic continuation prompt.
