...
- Runtime-health active, stale, and blocked task/project counts exclude archived projects. Archived task trees remain available through project/task history, but they must not continue inflating the operator's live workload summary after archival.
- Runtime-health `active_projects` specifically means projects with active work (queued, in progress, on hold, or live runtime ownership). A paused project with zero active work may still appear in project lists/history, but it does not count toward the active-work summary.
