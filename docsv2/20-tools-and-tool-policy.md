...
For resumed file-output recovery, the runtime must distinguish a real deliverable body from placeholder narration before treating `file.write` as progress. Substantive output means the candidate content materially advances the target file itself: concrete sections, lists, prose, code, or other body text that could reasonably remain on disk as the deliverable. Placeholder narration does not count, even when it is grammatically valid or longer than a one-line stub. Phrases that mainly announce the next step or the file's importance, such as "time to write the document", "this is the critical deliverable that unblocks the next stage", or similar setup/progress narration, must be rejected and preserved in the recovery checkpoint artifact instead of overwriting the target file as if useful work had landed. Inventory-style preambles also count as placeholder narration when they only list gathered context ("I have a deep understanding of..." plus bullets about corpus, voice, audience, goals, or architecture) and then announce that the real deliverable will be written next. If a later explicit resume produces the same class of non-substantive draft again, the runtime must escalate the checkpoint into an explicit hardened blocker contract: preserve the target path, artifact path, current failure reason, and prior failure reasons under `recovery_blocker_class=repeated_non_substantive_recovery_checkpoint`, and keep that contract authoritative for future `/v1/tasks/:id/resume` calls even when a legacy `chat_turn.stop_reason` constraint still forces the turn row itself to fall back to `model_error`. That same hardened contract applies when rejected work re-enters from `flow.rejected`: if a durable recovery checkpoint already exists, the next work turn must stay checkpoint-rooted instead of reopening broad exploratory tool use or generic kickoff narration.
...
For task-scoped flow sessions, `flow.get_execution` and `flow.review_decision` are anchored to the active execution bound to that session. If a review turn supplies the current review node id instead of the active `flow_node_execution_id`, the runtime resolves the active execution for that task/session rather than failing the decision with a misleading repo lookup error.
...
Recovery turns reopened after deterministic validation blockers, whether the kickoff came from an explicit task resume, a supervisor wakeup, or a `flow.rejected` work re-entry that still carries a durable recovery checkpoint, get one extra guardrail: if the provider emits `cli.execute` with no recoverable `command` at all, the turn runtime rejects that call before dispatch, injects one exact heredoc-style correction message back into the same turn, and gives the model one bounded retry. A second empty-command retry halts the turn with a diagnosable recovery message and, unless a real follow-on wakeup is already queued, moves the task into an explicit blocked state instead of leaving a silent `in_progress` dead end. When recent recovery context still identifies the intended file target, that halt must also persist the same `.ottercamp/recovery/...` artifact/checkpoint contract used by `file.write` recovery so the next resume gets a durable file-output anchor instead of a bare `model_error`.

Those same recovery turns apply the bounded pattern to `file.write` when a concrete target `path` exists but `content` is still empty or when recovery already supplied a candidate body in `file.write.content`. The runtime first looks for a substantive assistant draft already produced in the same turn and, if found, carries that draft directly into the write so document-generation tasks can still produce the artifact. Tool-repair chatter is not a substantive draft, and neither is shallow intent narration such as “Now I have everything I need. Let me write...” or checklist-style setup prose such as “Good, I now have ...”, “MUST-FIX”, “What’s needed”, or “Plan for ...” followed by a promise to write the document next: self-referential prose about missing parameters, conversation history, which tool to try next, the existing draft, gathered context, or merely announcing the plan to write the deliverable must never be written into the target file. If the only available text is that kind of repair chatter or placeholder preamble, runtime must halt with the same durable recovery artifact/checkpoint instead of treating the narration as file content or clobbering an existing on-disk draft. If the checkpoint target already contains a substantive deliverable, recovery must be overwrite-safe: a non-substantive candidate draft may update the recovery artifact/checkpoint, but it must not replace the existing target file. If a later explicit resume already carries a checkpoint saying the last draft was non-substantive, the resumed turn must harden itself around that fact: omit any target/artifact excerpt that matches the rejected placeholder pattern, treat the rejected text as invalid context instead of draft material, and require the next attempt to start with the actual file body. That same write-only recovery mode must also activate when a substantive target draft already exists but the remaining recovery artifact/checkpoint draft is still rejected placeholder narration; in that case the substantive target becomes the only valid revision baseline and the rejected artifact excerpt must stay out of the prompt. The same rule applies when the target file does not exist yet: if the only durable artifact is already rejected placeholder/setup prose, including template/style assets such as `templates/.../styles.css`, the resumed turn must still enter write-only recovery mode immediately instead of falling back into another empty `file.write` loop. That write-only recovery mode must activate as soon as the checkpoint rejects the prior draft and there is still no substantive durable target/artifact draft available; it must not wait for a second live failure simply because the rejected drafts were non-empty. In the repeated non-substantive blocker state, the same write-only recovery mode still applies, even if the target file already contains a substantive draft: root prompt history at the recovery checkpoint instead of the original kickoff, disable memory expansion, expose no tools to the model, and require the assistant to emit only the target file body as plain text so runtime can perform the final `file.write` itself. If the resumed turn still emits another semantically equivalent non-substantive placeholder, runtime must persist a stronger repeated-draft blocker reason on the checkpoint/artifact and stop there instead of burning the attempt down into generic `model_error`. A recovered write is not complete until the runtime can verify the target file exists durably on disk; a nominal tool result with no file on disk is a recovery failure, not a success. If that recovered write does land durably on the checkpoint target path, runtime must mark the `tool_result` as a completed-work signal for that turn, clear any stale in-memory recovery blocker carried over from the resume checkpoint, and let the normal task work completion path advance flow instead of enqueueing another generic recovery continuation. If the project still has a legacy org-nested workspace root from before the flattened layout, that durability check must accept the file from either compatible root instead of falsely re-blocking the task. In that case the turn must halt immediately, persist the draft plus the last write failure in `.ottercamp/recovery/...` when possible, and record the same failure reason in task checkpoint metadata so the next resume knows why the write did not land. If no draft exists yet, the runtime injects one correction that requires the full file body before another mutation attempt. A second empty-content retry halts the turn and persists a recovery checkpoint contract in task metadata. If the runtime can write a resumable artifact under `.ottercamp/recovery/...`, it must do that before telling the next turn to resume from the artifact; if it cannot, the halt message must use a truthful no-artifact contract instead of pointing at a non-existent file. When both the canonical flat workspace root and a legacy org-nested root already exist for the same project, the recovery artifact must be mirrored into both locations before the task metadata claims that artifact path exists. If a later explicit resume finds that the task was blocked truthfully and the durable target/artifact files still exist but the checkpoint metadata was dropped, the resume path must rebuild the checkpoint from that verified on-disk state before returning `task_resume_blocked_without_resumable_state`. If those files are missing but the recovery kickoff/session metadata or recent recovery tool results still identify the target or artifact path, the runtime must rebuild the same checkpoint from that session state instead of collapsing to a generic non-resumable blocker. That halt should persist `recovery_content_required` when the deployed `chat_turn` schema accepts it, and otherwise fall back to `model_error` rather than crashing the turn on stop-reason persistence. If that checkpoint already exists on a later resume, the next turn must seed itself from the durable target-file draft first, then the persisted recovery artifact draft, then the checkpoint metadata/failure reason so recovery stays bounded to real on-disk state instead of restarting contextless. If a later resumed turn still exhausts the continuation-depth guardrail budget before it can replace that checkpoint with substantive output, it must keep pointing back to the same durable target/artifact state instead of overwriting the target with placeholder narration first. If no real follow-on wakeup is already queued, the task must also move out of misleading `in_progress` into `blocked` so the checkpoint is paired with a truthful bounded operator state. Supervisor recovery must not spin a new identical turn while that same unresolved checkpoint remains the task's current halted turn.

Recovery turns that never reach tool dispatch because prompt assembly stays compressed or prompt input keeps breaching guardrail size across the full continuation-depth budget must use the same truthful pattern. The runtime must halt with explicit guidance, suppress silent auto-continuation for that exhausted recovery loop, and either keep the task `in_progress` only when a real queued follow-on wakeup already exists or move it to `blocked` with instructions to narrow the next resume attempt. If task metadata already carries a recovery checkpoint, the guardrail halt must point the operator back to that checkpoint, continuations must carry the same deterministic recovery-state summary instead of a generic "what were we doing?" summary, and any queued-wakeup claim must exclude the worker job that is already executing the current turn. If the metadata checkpoint was lost but the same recovery session still exposes the target/artifact path through kickoff metadata or recent file-output tool results, the guardrail halt must re-persist that checkpoint before the task lands in `blocked`.
