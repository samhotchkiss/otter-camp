...
func TestSupervisor_DurableRecoveryCheckpointTaskStaysBlockedForManualRepair(t *testing.T) {
	ctx := context.Background()
	pool := testdb.New(t)
	org := seedControlPlaneOrg(t, ctx, pool)
	projectRecord, _ := seedRunProjectTaskWithPM(t, ctx, pool, org.ID)
	worker := seedSupervisorAgent(t, ctx, pool, org.ID, "Manual Repair Worker")
	template := seedTaskQueueFlowTemplate(t, ctx, pool, org.ID, projectRecord.ID)
	if template.StartNodeID == nil || *template.StartNodeID == uuid.Nil {
		t.Fatal("expected executable flow template start node")
	}
	...
	taskRecord, err := taskRepo.Create(ctx, repo.ProjectTask{
		OrganizationID:    org.ID,
		ProjectID:         projectRecord.ID,
		Title:             "Blocked durable recovery checkpoint task",
		WorkStatus:        "blocked",
		FlowTemplateID:    &template.ID,
		CurrentFlowNodeID: template.StartNodeID,
		AssignedAgentID:   &worker.ID,
	...
	if _, err := eventRepo.Record(ctx, repo.ProjectTaskEvent{
		...
		Payload: json.RawMessage(fmt.Sprintf(`{
			...
			"recovery_checkpoint_target_path":"templates/layout-07-split-screen.html",
			"recovery_checkpoint_artifact_path":".ottercamp/recovery/templates/layout-07-split-screen.html",
			"recovery_checkpoint_failure_reason":"file.write content appears to be task narration about planning to write the deliverable, not the deliverable body itself. Write the concrete file contents directly."
		}`, blockerReason)),
	}); err != nil {
		t.Fatalf("Record status.changed: %v", err)
	}
	if _, err := pool.Exec(ctx, `UPDATE project_task_event SET created_at = $2 WHERE task_id = $1 AND event_type = 'status.changed' AND COALESCE(payload->>'to_status','') = 'blocked'`, taskRecord.ID, time.Now().UTC().Add(-2*time.Minute)); err != nil {
...
func TestSupervisor_InheritedSharedDeliverableChildTaskStaysBlocked(t *testing.T) {
	ctx := context.Background()
	pool := testdb.New(t)
	org := seedControlPlaneOrg(t, ctx, pool)
	projectRecord, _ := seedRunProjectTaskWithPM(t, ctx, pool, org.ID)
	worker := seedSupervisorAgent(t, ctx, pool, org.ID, "Shared Deliverable Worker")
	template := seedTaskQueueFlowTemplate(t, ctx, pool, org.ID, projectRecord.ID)
	if template.StartNodeID == nil || *template.StartNodeID == uuid.Nil {
		t.Fatal("expected executable flow template start node")
	}
	...
	taskRecord, err := taskRepo.Create(ctx, repo.ProjectTask{
		OrganizationID:    org.ID,
		ProjectID:         projectRecord.ID,
		Title:             "Inherited shared deliverable child task",
		WorkStatus:        "blocked",
		FlowTemplateID:    &template.ID,
		CurrentFlowNodeID: template.StartNodeID,
		AssignedAgentID:   &worker.ID,
		CreatedByType:     "system",
		Metadata: json.RawMessage(`{
			"decomposition_parent_task_id":"` + `00000000-0000-0000-0000-000000000001` + `",
	...
	blockerReason := "recovery halted because the decomposed child lane inherits shared parent deliverable planning/sambot-architecture.md; resume only with bounded file.edit updates on the current shared file or by moving integration back to the parent task"
	eventRepo := repo.NewProjectTaskEventRepo(pool)
	if _, err := eventRepo.Record(ctx, repo.ProjectTaskEvent{
		TaskID:     taskRecord.ID,
		ProjectID:  projectRecord.ID,
	...
	if guard.FailureCode != "inherited_shared_deliverable_write_blocked" {
		t.Fatalf("validation failure_code = %q, want inherited_shared_deliverable_write_blocked", guard.FailureCode)
	}
	updatedExecution, err := repo.NewFlowNodeExecutionRepo(pool).GetByID(ctx, execution.ID)
	if err != nil {
		t.Fatalf("GetByID execution: %v", err)
...
func TestSupervisor_TerminalMalformedCheckpointChildTaskStaysBlocked(t *testing.T) {
	ctx := context.Background()
	pool := testdb.New(t)
	org := seedControlPlaneOrg(t, ctx, pool)
	projectRecord, _ := seedRunProjectTaskWithPM(t, ctx, pool, org.ID)
	worker := seedSupervisorAgent(t, ctx, pool, org.ID, "Malformed Checkpoint Worker")
	template := seedTaskQueueFlowTemplate(t, ctx, pool, org.ID, projectRecord.ID)
	if template.StartNodeID == nil || *template.StartNodeID == uuid.Nil {
		t.Fatal("expected executable flow template start node")
	}
	...
	taskRecord, err := taskRepo.Create(ctx, repo.ProjectTask{
		OrganizationID:    org.ID,
		ProjectID:         projectRecord.ID,
		Title:             "Duplicate shared-file child task",
		WorkStatus:        "blocked",
		FlowTemplateID:    &template.ID,
		CurrentFlowNodeID: template.StartNodeID,
		AssignedAgentID:   &worker.ID,
	...
	blockerReason := "OC-152 was created as a child of OC-150 but still duplicates the parent's shared single-file deliverable instead of narrowing the work; resume the bounded split from the parent task or create a real section-scoped child instead of this malformed lane"
	checkpointMetadata, err := taskcheckpoint.MergeRecoveryFileWriteCheckpoint(taskRecord.Metadata, taskcheckpoint.RecoveryFileWriteCheckpoint{
	...
	if _, err := eventRepo.Record(ctx, repo.ProjectTaskEvent{
		...
		Payload: json.RawMessage(fmt.Sprintf(`{
			...
			"recovery_checkpoint_failure_reason":%q
		}`, blockerReason, blockerReason)),
	}); err != nil {
		t.Fatalf("Record status.changed: %v", err)
	}
...
func TestSupervisor_FlowRejectionMaxVisitsBlockedTaskStaysBlocked(t *testing.T) {
	ctx := context.Background()
	pool := testdb.New(t)
	org := seedControlPlaneOrg(t, ctx, pool)
	projectRecord, _ := seedRunProjectTaskWithPM(t, ctx, pool, org.ID)
	worker := seedSupervisorAgent(t, ctx, pool, org.ID, "Flow Rejection Worker")
	template := seedTaskQueueFlowTemplate(t, ctx, pool, org.ID, projectRecord.ID)
	if template.StartNodeID == nil || *template.StartNodeID == uuid.Nil {
		t.Fatal("expected executable flow template start node")
	}
	...
	taskRecord, err := taskRepo.Create(ctx, repo.ProjectTask{
		OrganizationID:    org.ID,
		ProjectID:         projectRecord.ID,
		Title:             "Blocked flow rejection task",
		WorkStatus:        "blocked",
		FlowTemplateID:    &template.ID,
		CurrentFlowNodeID: &reviewNode.ID,
		AssignedAgentID:   &worker.ID,
		CreatedByType:     "system",
		Metadata:          json.RawMessage(`{}`),
	})
	...
	eventRepo := repo.NewProjectTaskEventRepo(pool)
	blockedEventTime := time.Now().UTC().Add(-2 * time.Minute)
	...
	if _, err := pool.Exec(ctx, `UPDATE project_task_event SET created_at = $2 WHERE task_id = $1 AND event_type = 'status.changed' AND COALESCE(payload->>'to_status','') = 'blocked'`, taskRecord.ID, blockedEventTime); err != nil {
		t.Fatalf("Update blocked event created_at: %v", err)
	}
	if _, err := eventRepo.Record(ctx, repo.ProjectTaskEvent{
		TaskID:     taskRecord.ID,
	...
	if _, err := pool.Exec(ctx, `UPDATE project_task_event SET created_at = $2 WHERE task_id = $1 AND event_type = 'flow.rejected'`, taskRecord.ID, flowRejectedTime); err != nil {
