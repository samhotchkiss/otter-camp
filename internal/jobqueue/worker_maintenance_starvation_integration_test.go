...
func TestJobWorkerReservesExecutionSlotWhenMemoryExtractAlreadyInFlight(t *testing.T) {
	pool := testdb.New(t)
	worker := New(pool, nil, Config{
		BatchSize:            4,
		PollInterval:         20 * time.Millisecond,
	...
	worker.Register(memoryExtractTurnJobType, func(context.Context, Job) error {
		select {
		case maintenanceStarted <- struct{}{}:
		default:
		}
	...
	for i := 0; i < 6; i++ {
		if _, err := worker.Enqueue(context.Background(), nil, memoryExtractTurnJobType, 60, map[string]any{
			"session_id": uuid.New(),
			"turn_id":    uuid.New(),
		}, nil); err != nil {
			t.Fatalf("enqueue memory extract %d failed: %v", i+1, err)
		}
	}
	...
	for i := 0; i < 3; i++ {
		...
		case <-maintenanceStarted:
		case <-time.After(2 * time.Second):
			t.Fatalf("memory extract job %d did not start", i+1)
		}
	}

	select {
	case <-maintenanceStarted:
		t.Fatal("memory extract jobs filled all worker slots before agent work arrived")
	default:
	}

	org, err := repo.NewOrgRepo(pool).Create(context.Background(), repo.Organization{
		Slug:        "memory-extract-slot-reservation",
		DisplayName: "Memory Extract Slot Reservation",
	})
	if err != nil {
		t.Fatalf("create org: %v", err)
	}
	...
	case <-agentStarted:
	case <-time.After(2 * time.Second):
		t.Fatal("agent_turn did not start while memory extract jobs were already in flight")
	}
