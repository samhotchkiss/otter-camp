...
func TestOperatorDashboardIgnoresArchivedProjectsInPausedAndHumanInputBlockedCounts(t *testing.T) {
	t.Setenv("OTTERCAMP_AUTH_MODE", "standard")
	...
	project, err := projectRepo.Create(ctx, repo.Project{
		OrganizationID: orgA.ID,
		Slug:           "ops-dashboard-archived-paused-input",
		DisplayName:    "Ops Dashboard Archived Paused Input",
		DeliveryMode:   "gated",
		CreatedByType:  "human_user",
		CreatedByID:    adminA.ID,
		Settings:       json.RawMessage(`{"pause":{"is_paused":true,"reason":"archived project should be hidden"}}`),
	})
	if err != nil {
		t.Fatalf("create project: %v", err)
	}
	if _, err := inboxRepo.Create(ctx, repo.InboxItem{
		...
		Title:           "Archived approval request",
		ActionPayload:   json.RawMessage(`{"kind":"approval"}`),
	}); err != nil {
	...
	if item := findOperatorDashboardItem(payload.Blocked.Items, "paused_project", project.ID); item != nil {
		t.Fatalf("paused archived project should be hidden: %+v body=%s", *item, string(resp.Body))
	}
	if item := findOperatorDashboardItem(payload.Blocked.Items, "human_approval_required", project.ID); item != nil {
		t.Fatalf("human input for archived project should be hidden: %+v body=%s", *item, string(resp.Body))
	}
}
