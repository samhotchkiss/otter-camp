...
// TestCreateProjectSlugAutoGenerated verifies that when no slug is provided,
// the handler auto-generates one from display_name and calls the service.
func TestCreateProjectSlugAutoGenerated(t *testing.T) {
	...
	if svc.createCalls != 1 {
		t.Fatalf("create calls = %d, want 1 (slug should be auto-generated)", svc.createCalls)
	}
}

func TestAddFlowNodeParsesLabelAndOrdinalRequestFields(t *testing.T) {
	orgID := uuid.New()
	templateID := uuid.New()
	svc := &fakeProjectService{
		flowTemplate: &projectsvc.FlowTemplate{
			ID:             templateID,
	...
	req := newProjectRequest(t, http.MethodPost, "/v1/flow-templates/"+templateID.String()+"/nodes", map[string]any{
		"label":     "Code",
		"node_type": "work",
		"ordinal":   1,
	})
	req = withRouteParams(req, map[string]string{"id": templateID.String()})
	rr := httptest.NewRecorder()

	handlers.addFlowNode(rr, req)

	if rr.Code != http.StatusCreated {
		t.Fatalf("status = %d, want %d body=%s", rr.Code, http.StatusCreated, rr.Body.String())
	}
	if svc.addFlowNodeCalls != 1 {
		t.Fatalf("AddFlowNode calls = %d, want 1", svc.addFlowNodeCalls)
	}
	if svc.lastAddFlowNodeReq.DisplayName != "Code" {
		t.Fatalf("DisplayName = %q, want %q", svc.lastAddFlowNodeReq.DisplayName, "Code")
	}
	if svc.lastAddFlowNodeReq.NodeType != "work" {
		t.Fatalf("NodeType = %q, want %q", svc.lastAddFlowNodeReq.NodeType, "work")
	}
	if svc.lastAddFlowNodeReq.Position != 1 {
		t.Fatalf("Position = %d, want %d", svc.lastAddFlowNodeReq.Position, 1)
	}
	...
	if label, _ := data["label"].(string); label != "Code" {
		t.Fatalf("response label = %q, want %q", label, "Code")
	}
...
func (f *fakeProjectService) GetFlowTemplate(context.Context, uuid.UUID, uuid.UUID) (*projectsvc.FlowTemplate, error) {
	if f.flowTemplate != nil {
		return f.flowTemplate, nil
	}
	return nil, repo.ErrNotFound
}
...
func (f *fakeProjectService) AddFlowNode(_ context.Context, templateID uuid.UUID, req projectsvc.AddFlowNodeRequest) (*projectsvc.FlowNode, error) {
	f.addFlowNodeCalls++
	f.lastAddFlowNodeReq = req
	if f.addFlowNodeErr != nil {
		return nil, f.addFlowNodeErr
	}
	if f.addFlowNodeResult != nil {
		return f.addFlowNodeResult, nil
	}
	return &projectsvc.FlowNode{
		ID:             uuid.New(),
