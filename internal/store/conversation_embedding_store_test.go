...
func TestConversationEmbeddingQueueListAndUpdate1536Dimension(t *testing.T) {
	connStr := getTestDatabaseURL(t)
	db := setupTestDatabase(t, connStr)

	orgID := createTestOrganization(t, db, "conversation-embedding-queue-1536-org")
	projectID := createTestProject(t, db, orgID, "Conversation Embedding Queue 1536 Project")
	agentID := insertSchemaAgent(t, db, orgID, "conversation-embedding-queue-1536-agent")

	var roomID string
	err := db.QueryRow(
		`INSERT INTO rooms (org_id, name, type, context_id)
		 VALUES ($1, 'Queue 1536 Room', 'project', $2)
		 RETURNING id`,
		orgID,
		projectID,
	).Scan(&roomID)
	...
	err = db.QueryRow(
		`INSERT INTO chat_messages (org_id, room_id, sender_id, sender_type, body, type, attachments)
		 VALUES ($1, $2, $3, 'agent', 'pending 1536 chat embedding', 'message', '[]'::jsonb)
		 RETURNING id`,
		orgID,
		roomID,
		agentID,
	...
	err = db.QueryRow(
		`INSERT INTO memories (org_id, kind, title, content, status)
		 VALUES ($1, 'fact', 'Pending 1536 memory title', 'pending 1536 memory embedding', 'active')
		 RETURNING id`,
		orgID,
	).Scan(&memoryID)
	require.NoError(t, err)

	queue := NewConversationEmbeddingStoreWithDimension(db, 1536)
	...
	var chatHasEmbedding1536 bool
	err = db.QueryRow(`SELECT embedding_1536 IS NOT NULL FROM chat_messages WHERE id = $1`, chatMessageID).Scan(&chatHasEmbedding1536)
	require.NoError(t, err)
	require.True(t, chatHasEmbedding1536)

	var memoryHasEmbedding1536 bool
	err = db.QueryRow(`SELECT embedding_1536 IS NOT NULL FROM memories WHERE id = $1`, memoryID).Scan(&memoryHasEmbedding1536)
	require.NoError(t, err)
	require.True(t, memoryHasEmbedding1536)
}
