...
func TestProjectIssueStore_TransitionWorkStatusSupportsLifecyclePipeline(t *testing.T) {
	...
	planning, err := issueStore.TransitionWorkStatus(ctx, issue.ID, IssueWorkStatusInProgress)
	require.NoError(t, err)
	require.Equal(t, IssueWorkStatusInProgress, planning.WorkStatus)
	require.Equal(t, "open", planning.State)

	readyForWork, err := issueStore.TransitionWorkStatus(ctx, issue.ID, IssueWorkStatusInProgress)
	require.NoError(t, err)
	require.Equal(t, IssueWorkStatusInProgress, readyForWork.WorkStatus)
	...
	flagged, err := issueStore.TransitionWorkStatus(ctx, issue.ID, IssueWorkStatusBlocked)
	require.NoError(t, err)
	require.Equal(t, IssueWorkStatusBlocked, flagged.WorkStatus)
