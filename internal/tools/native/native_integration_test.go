...
func TestIntegrationFileListAllowsBlockedReviewNodeDeliverableRootInspectionWithinRecoveryTargetRoot(t *testing.T) {
	pool := testdb.New(t)
	ctx := context.Background()
	orgID := testutil.MakeOrg(t, pool)
	project := testutil.MakeProject(t, pool, orgID)
	flowTemplate := testutil.MakeFlowTemplate(t, pool, project.ID, 1)
	description := "Read content/technonymous-index.json. For each of the next 12 URLs in the post_urls array (indices 12–23), use web_fetch to retrieve the page content, then save the article text as a clean Markdown file under content/posts/. Deliverable: 12 markdown files in content/posts/."
	plan := taskplan.Analyze("Fetch posts 13-24 from Technonymous index", &description)
	task := testutil.MakeTask(t, pool, project.ID, testutil.MakeTaskOptions{
		Title:          "Fetch posts 13-24 from Technonymous index",
		Description:    &description,
		Metadata:       taskplan.ApplyMetadata(nil, plan),
		FlowTemplateID: &flowTemplate.ID,
		WorkStatus:     "blocked",
	...
	nodes, err := nodeRepo.GetByTemplateOrdered(ctx, flowTemplate.ID)
	if err != nil {
		t.Fatalf("load flow nodes: %v", err)
	}
	if len(nodes) != 1 {
		t.Fatalf("flow node count = %d, want 1", len(nodes))
	}
	reviewNode := nodes[0]
	...
	if _, err := repo.NewProjectTaskRepo(pool).SetFlowNode(ctx, task.ID, &reviewNode.ID); err != nil {
	...
	if _, err := repo.NewChatMessageRepo(pool).Create(ctx, repo.ChatMessage{
		...
		Role:      "system",
		Status:    "final",
		Content:   "[Recovery resume state]\nTarget file: content/posts/mister-rogers-and-the-forgotten-art.md\n",
	}); err != nil {
		t.Fatalf("create recovery system message: %v", err)
	}
	...
	if err := os.WriteFile(filepath.Join(workspaceRoot, "content", "posts", "mister-rogers-and-the-forgotten-art.md"), []byte("# Mister Rogers and the Forgotten Art\n"), 0o644); err != nil {
		t.Fatalf("write target post: %v", err)
	}
	if err := os.WriteFile(filepath.Join(workspaceRoot, "content", "posts", "practice-listen-to-albums.md"), []byte("# Practice listen to albums\n"), 0o644); err != nil {
		t.Fatalf("write sibling post: %v", err)
	}

	out, err := executor.Execute(execCtx, "file.list", map[string]any{"path": "content/posts"})
	if err != nil {
		t.Fatalf("file.list: %v", err)
	}
	if got := out["error"]; got != nil {
...
func TestIntegrationFileListRejectsExecutionDeliverableRootInspectionWithinRecoveryTargetRootForCheckpointTask(t *testing.T) {
	pool := testdb.New(t)
	ctx := context.Background()
	orgID := testutil.MakeOrg(t, pool)
	project := testutil.MakeProject(t, pool, orgID)
	...
	if got := out["deliverable_path"]; got != "content/posts/mister-rogers-and-the-forgotten-art.md" {
		t.Fatalf("deliverable_path = %v, want %q", got, "content/posts/mister-rogers-and-the-forgotten-art.md")
	}
}
...
func TestIntegrationFileSearchOmitsPlaceholderRecoveryTargetWithoutExplicitDeliverable(t *testing.T) {
	pool := testdb.New(t)
	orgID := testutil.MakeOrg(t, pool)
	project := testutil.MakeProject(t, pool, orgID)
	description := "Consolidate all validation findings into a single report. Categorize issues by severity and prepare for review."
	...
	otherPath := filepath.Join(dataDir, "workspaces", projectRecord.Slug, "Work", "EXECUTION-SUMMARY.md")
	if err := os.WriteFile(otherPath, []byte("Prior tasks completed and artifacts are ready."), 0o644); err != nil {
		t.Fatalf("write other file: %v", err)
	}

	out, err := executor.Execute(ctx, "file.search", map[string]any{
		"path":    "Work",
		"pattern": "Prior tasks completed",
	})
	if err != nil {
		t.Fatalf("file.search: %v", err)
	}
	...
	if got := matches[0]["file"]; got != "Work/EXECUTION-SUMMARY.md" {
		t.Fatalf("match file = %v, want Work/EXECUTION-SUMMARY.md", got)
	}
}
...
func TestIntegrationFileReadRejectsMarkdownReviewAssessmentPlaceholderAtInProgressDeliverablePath(t *testing.T) {
	pool := testdb.New(t)
	orgID := testutil.MakeOrg(t, pool)
	project := testutil.MakeProject(t, pool, orgID)
...
func TestIntegrationTaskUpdateAcceptsReadyAliasForQueued(t *testing.T) {
	...
	if !ok {
		t.Fatalf("task.update ready alias task = %T, want map[string]any", out["task"])
	}
	if got := strings.TrimSpace(fmt.Sprintf("%v", taskOut["work_status"])); got != "queued" {
...
func TestIntegrationTaskUpdateAcceptsWorkAliasForQueued(t *testing.T) {
	pool := testdb.New(t)
	backgroundCtx := context.Background()
	orgID := testutil.MakeOrg(t, pool)
	project := testutil.MakeProject(t, pool, orgID)
	...
	if err != nil {
		t.Fatalf("task.update work alias: %v", err)
	}
	if out["error"] != nil {
		t.Fatalf("task.update work alias error = %v, want nil", out["error"])
	}
	taskOut, ok := out["task"].(map[string]any)
	if !ok {
		t.Fatalf("task.update work alias task = %T, want map[string]any", out["task"])
	}
	if got := strings.TrimSpace(fmt.Sprintf("%v", taskOut["work_status"])); got != "queued" {
		t.Fatalf("work_status = %q, want queued", got)
	}
...
func TestIntegrationProjectSessionTaskCreateRejectsMetaReviewTaskWhenDraftsRemain(t *testing.T) {
	...
	if got := fmt.Sprintf("%v", out["error"]); got != projectContinuationMetaTaskCreateError {
		t.Fatalf("fourth task.create error = %q, want %q", got, projectContinuationMetaTaskCreateError)
	}
	...
	if got := fmt.Sprintf("%v", out["error"]); got != projectContinuationMetaTaskCreateError {
		t.Fatalf("seventh task.create error = %q, want %q", got, projectContinuationMetaTaskCreateError)
	}

	tasks, err = taskRepo.ListByProject(ctx, project.ID)
	if err != nil {
		t.Fatalf("list project tasks after seventh variant: %v", err)
	}
	if len(tasks) != 2 {
	...
	if got := fmt.Sprintf("%v", out["error"]); got != projectContinuationMetaTaskCreateError {
		t.Fatalf("ninth task.create error = %q, want %q", got, projectContinuationMetaTaskCreateError)
	}

	tasks, err = taskRepo.ListByProject(ctx, project.ID)
	if err != nil {
		t.Fatalf("list project tasks after ninth variant: %v", err)
	}
	if len(tasks) != 2 {
		t.Fatalf("project task count = %d after ninth variant, want 2", len(tasks))
	}
}
...
func TestIntegrationTaskUpdateQueueAllowsConcreteCrawlerTaskWithExactStepsAndNoDecompose(t *testing.T) {
	pool := testdb.New(t)
	ctx := context.Background()
	orgID := testutil.MakeOrg(t, pool)
	project := testutil.MakeProject(t, pool, orgID)
	...
	sessionOut, err := NewExecutor(ExecutorOptions{Pool: pool, WorkspaceRoot: t.TempDir()}).Execute(integrationExecCtxWith(orgID, agent.ID), "session.create", map[string]any{
		...
		"scope_id":   project.ID.String(),
		"mode":       "async",
		"title":      "Queue technonymous index crawl",
	})
	if err != nil {
		t.Fatalf("session.create: %v", err)
	}
	...
	for _, projectTask := range tasks {
		if taskdecomp.ParseParentTaskID(projectTask.Metadata) == task.ID {
			t.Fatalf("unexpected decomposed child task persisted: %s", projectTask.Title)
		}
	}
}
...
func TestIntegrationFlowReviewDecisionRejectCreatesCanonicalRejectionCommit(t *testing.T) {
	...
	template, err := templateRepo.Create(ctx, repo.FlowTemplate{
		OrganizationID: &projectRecord.OrganizationID,
		ProjectID:      &project.ID,
		Slug:           "reject-commit-flow-" + strings.ToLower(uuid.NewString()[:8]),
		DisplayName:    "Reject Commit Flow " + uuid.NewString()[:8],
		Description:    "test flow template for rejection commits",
		IsCurrent:      true,
		Version:        1,
		CreatedByType:  "system",
		CreatedByID:    uuid.Nil,
	...
	if out["next_node_id"] == nil {
		t.Fatalf("next_node_id = %v, want new work node id", out["next_node_id"])
	}
	...
	message := strings.TrimSpace(string(messageBytes))
	if !strings.Contains(message, "flow(review:review#1): reject - missing tests") {
		t.Fatalf("latest commit message = %q, want rejection commit header", message)
	}
}
