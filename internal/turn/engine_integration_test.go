...
type failSpecificQueueTransitionService struct {
	base       taskTransitionService
...
func (f *failSpecificQueueTransitionService) TransitionStatus(ctx context.Context, taskID uuid.UUID, toStatus string, actor tasksvc.Actor) (*tasksvc.ProjectTask, error) {
	...
	if taskID == f.failTaskID && strings.EqualFold(strings.TrimSpace(toStatus), "queued") {
		return nil, taskdecomp.ErrBoundedTaskTooLarge
	}
	return f.base.TransitionStatus(ctx, taskID, toStatus, actor)
}

func (f *failSpecificQueueTransitionService) TransitionStatusWithPayload(ctx context.Context, taskID uuid.UUID, toStatus string, actor tasksvc.Actor, extraPayload map[string]any) (*tasksvc.ProjectTask, error) {
	if f.base == nil {
		return nil, errors.New("missing base transition service")
	}
	if taskID == f.failTaskID && strings.EqualFold(strings.TrimSpace(toStatus), "queued") {
	...
	return f.base.TransitionStatusWithPayload(ctx, taskID, toStatus, actor, extraPayload)
}

func (f *failSpecificQueueTransitionService) MarkBlocked(ctx context.Context, taskID uuid.UUID, reason string, actor tasksvc.Actor) (*tasksvc.ProjectTask, error) {
	if f.base == nil {
		return nil, errors.New("missing base transition service")
	}
...
func TestTurnEngineIntegrationProjectSessionKickoffRoutesFirstJobToFrank(t *testing.T) {
	fixture := newIntegrationFixture(t)
	ctx := context.Background()

	frank := fixture.agent
	_ = mustCreateStarterLori(t, ctx, fixture.pool, fixture.org.ID)
	project := mustCreateProject(t, ctx, fixture.pool, fixture.org.ID, fixture.user.ID)
	...
	userMessage, err := fixture.chatService.AppendMessage(ctx, chat.AppendMessageInput{
		...
		AuthorID:   &fixture.user.ID,
		Role:       "user",
		Content:    "start a fresh project kickoff",
	})
	if err != nil {
		t.Fatalf("AppendMessage: %v", err)
	}
	...
	payload := mustLatestAgentTurnPayloadForSession(t, ctx, fixture.pool, projectSession.ID)
	if payload.AgentID == nil {
		t.Fatalf("payload.agent_id = nil, want %s", frank.ID)
	}
	if *payload.AgentID != frank.ID {
		t.Fatalf("payload.agent_id = %s, want %s", *payload.AgentID, frank.ID)
	}
	...
	hasFrank := false
	for _, participant := range participants {
		if participant != nil && participant.ParticipantType == "agent" && participant.ParticipantID == frank.ID {
			hasFrank = true
			break
		}
	}
	if !hasFrank {
		t.Fatalf("expected Frank %s in session participants", frank.ID)
	}
}

func TestTurnEngineIntegrationProjectSessionKickoffHandsOffToLoriAfterCompletedFrankTurn(t *testing.T) {
	fixture := newIntegrationFixture(t)
	ctx := context.Background()
	...
	priorTurn, err := fixture.chatService.CreateTurn(ctx, projectSession.ID, frank.ID)
	if err != nil {
		t.Fatalf("CreateTurn prior Frank kickoff: %v", err)
	}
	if err := fixture.chatService.StartTurn(ctx, priorTurn.ID); err != nil {
		t.Fatalf("StartTurn prior Frank kickoff: %v", err)
	}
	if err := fixture.chatService.CompleteTurn(ctx, priorTurn.ID); err != nil {
		t.Fatalf("CompleteTurn prior Frank kickoff: %v", err)
	}
	...
	userMessage, err := fixture.chatService.AppendMessage(ctx, chat.AppendMessageInput{
		...
		AuthorID:   &fixture.user.ID,
		Role:       "user",
		Content:    "continue bootstrap setup",
	})
	if err != nil {
		t.Fatalf("AppendMessage: %v", err)
	}
	...
	payload := mustLatestAgentTurnPayloadForSession(t, ctx, fixture.pool, projectSession.ID)
	if payload.AgentID == nil {
		t.Fatalf("payload.agent_id = nil, want %s", lori.ID)
	}
	if *payload.AgentID != lori.ID {
		t.Fatalf("payload.agent_id = %s, want %s", *payload.AgentID, lori.ID)
	}
	...
	hasLori := false
	for _, participant := range participants {
		if participant != nil && participant.ParticipantType == "agent" && participant.ParticipantID == lori.ID {
			hasLori = true
			break
		}
	}
	if !hasLori {
		t.Fatalf("expected Lori %s in session participants", lori.ID)
	}
}
...
func TestTurnEngineIntegrationFreshProjectHandoffBlocksMemoryTools(t *testing.T) {
	fixture := newIntegrationFixture(t)
	ctx := context.Background()

	project := mustCreateProject(t, ctx, fixture.pool, fixture.org.ID, fixture.user.ID)
	project.Slug = "sam-blog-fresh-memory-guard"
	...
	fixture.engine.toolResolver = &fakeToolResolver{tools: []tools.ToolDescriptor{
		{Name: "memory.search", Tier: "tier1"},
	}}
	fixture.model.streamFn = func(_ context.Context, _ ModelRequest, _ func(token string) error) (ModelResponse, error) {
		return ModelResponse{ToolCalls: []ModelToolCall{
			{ID: "memory-1", Name: "memory.search", Tier: "tier1", Arguments: map[string]any{"query": "Sam.blog prior run"}},
		}}, nil
	}
	...
	foundBlockedMemory := false
	for _, message := range messages {
		if !strings.EqualFold(strings.TrimSpace(message.Role), "tool_result") || message.ToolCallID == nil || *message.ToolCallID != "memory-1" {
			continue
		}
		if strings.Contains(message.Content, "fresh kickoff should rely on the current request") {
			foundBlockedMemory = true
			break
		}
	}
	if !foundBlockedMemory {
		t.Fatal("missing blocked fresh-kickoff memory tool result")
	}
}
...
func TestTurnEngineIntegrationRecoveryResumeUsesPersistedArtifactDraftAfterRepeatedIntentOnlyDraftEX329(t *testing.T) {
	fixture := newIntegrationFixture(t)
	ctx := context.Background()
...
`)
	...
	if modelCalls != 2 {
		t.Fatalf("model calls = %d, want 2 after persisted-draft recovery write", modelCalls)
	}
	if dispatched != 1 {
		t.Fatalf("tier2 dispatches = %d, want 1", dispatched)
	}
	...
	if string(outputBody) != artifactDraft {
		t.Fatalf("target file body = %q, want persisted artifact draft", string(outputBody))
	}
	...
	if lastTurn.StopReason != nil && strings.TrimSpace(*lastTurn.StopReason) == stopReasonRecoveryFileRejected {
		t.Fatalf("turn stop_reason = %q, want persisted draft write to proceed", strings.TrimSpace(*lastTurn.StopReason))
	}
	...
	var successfulWrites int
	for _, item := range messages {
		if item.Role == "tool_result" && strings.Contains(item.Content, `"path":"docs/content-strategy.md"`) && strings.Contains(item.Content, `"created":true`) {
			successfulWrites++
		}
	...
	if haltMessages != 0 {
		t.Fatalf("hardened halt messages = %d, want 0", haltMessages)
	}
	if successfulWrites != 1 {
		t.Fatalf("successful file.write tool_results = %d, want 1", successfulWrites)
	}
}
...
func TestTurnEngineIntegrationRecoveryResumeReplacesIntentOnlyFileWriteContentFromPersistedArtifactDraft(t *testing.T) {
	...
	recoveryMessage, err := fixture.chatService.AppendMessage(ctx, chat.AppendMessageInput{
		...
		Metadata: mustJSON(t, map[string]any{
			...
			"validation_failure_reason": "content is required",
		}),
	})
	if err != nil {
		t.Fatalf("AppendMessage recovery kickoff: %v", err)
	...
	fixture.model.streamFn = func(_ context.Context, _ ModelRequest, _ func(token string) error) (ModelResponse, error) {
		modelCalls++
		if modelCalls > 2 {
			t.Fatalf("unexpected extra model call after persisted-draft replacement: %d", modelCalls)
		}
		if modelCalls == 2 {
			return ModelResponse{Content: "persisted recovery draft applied"}, nil
		}
		return ModelResponse{
			Content: "",
			ToolCalls: []ModelToolCall{{
				ID:   "resume-write-intent-only",
				Name: "file.write",
				Tier: "tier2",
				Arguments: map[string]any{
					"path":    seed.targetPath,
					"content": "I will write the complete content strategy document now.",
				},
			}},
		}, nil
	}
	...
	if err := fixture.engine.HandleUserMessage(ctx, taskSession.ID, recoveryMessage.ID); err != nil {
		t.Fatalf("HandleUserMessage persisted recovery draft replacement: %v", err)
	}

	if modelCalls != 2 {
		t.Fatalf("model calls = %d, want 2 after persisted-draft replacement", modelCalls)
	}
	if dispatched != 1 {
		t.Fatalf("tier2 dispatches = %d, want 1", dispatched)
	}
	...
	updatedTask, err := taskRepo.GetByID(ctx, taskRecord.ID)
	if err != nil {
		t.Fatalf("GetByID task after persisted-draft replacement: %v", err)
	}
	if updatedTask.WorkStatus == "blocked" {
		t.Fatalf("task work_status = %q, want not blocked", updatedTask.WorkStatus)
	}
	...
	for _, item := range messages {
		if item.Role == "system" && strings.Contains(item.Content, "Recovery turn halted: recovered file.write") {
			t.Fatalf("unexpected recovery halt message: %s", item.Content)
		}
	}
}

func TestTurnEngineIntegrationRecoveryResumeBlocksIntentOnlyNarrationBeforeReadOnlyToolDispatch(t *testing.T) {
	fixture := newIntegrationFixture(t)
	ctx := context.Background()
	...
	recoveryMessage, err := fixture.chatService.AppendMessage(ctx, chat.AppendMessageInput{
		...
		Metadata: mustJSON(t, map[string]any{
			...
			"recovery_action":           "resume_validation_blocked_task",
			"validation_tool_name":      "file.write",
			"validation_failure_code":   "non_substantive_content",
			"validation_failure_reason": "content appears to narrate intent instead of writing the deliverable body",
		}),
	})
	if err != nil {
		t.Fatalf("AppendMessage recovery kickoff: %v", err)
	...
	fixture.engine.toolResolver = &fakeToolResolver{tools: []tools.ToolDescriptor{{Name: "file.list", Tier: "tier1"}}}

	modelCalls := 0
	fixture.model.streamFn = func(_ context.Context, _ ModelRequest, _ func(token string) error) (ModelResponse, error) {
		modelCalls++
		if modelCalls > 1 {
			t.Fatalf("unexpected extra model call after rejected recovery narration: %d", modelCalls)
		}
		return ModelResponse{
			Content: "Let me first check the current state of the target file and recovery artifacts, then write the substantive content strategy document.",
			ToolCalls: []ModelToolCall{{
				ID:   "resume-read-only-list",
				Name: "file.list",
				Tier: "tier1",
				Arguments: map[string]any{
					"path": ".",
	...
	if err := fixture.engine.HandleUserMessage(ctx, taskSession.ID, recoveryMessage.ID); err != nil {
		t.Fatalf("HandleUserMessage rejected recovery narration: %v", err)
	}

	if dispatched != 0 {
		t.Fatalf("tier1 dispatches = %d, want 0", dispatched)
	}

	updatedTask, err := taskRepo.GetByID(ctx, taskRecord.ID)
	if err != nil {
		t.Fatalf("GetByID task after rejected recovery narration: %v", err)
	}
	if updatedTask.WorkStatus != "blocked" {
		t.Fatalf("task work_status = %q, want blocked", updatedTask.WorkStatus)
	}
	...
	if !sameWorkspaceRelativePath(checkpoint.TargetPath, seed.targetPath) {
		t.Fatalf("checkpoint target_path = %q, want %q", checkpoint.TargetPath, seed.targetPath)
	}
	if !strings.Contains(checkpoint.FailureReason, "instead of the file body") {
		t.Fatalf("checkpoint failure_reason = %q, want file-body rejection", checkpoint.FailureReason)
	}
	...
	for _, item := range messages {
		...
		if item.Role == "tool_result" && strings.Contains(item.Content, `"tool_name":"file.list"`) {
			t.Fatalf("unexpected file.list dispatch after rejected recovery narration: %s", item.Content)
		}
	}
	if !foundHalt {
...
func TestTurnEngineIntegrationProjectBootstrapFailsClosedWhenFirstWaveQueueHitsBoundedSize(t *testing.T) {
	fixture := newIntegrationFixture(t)
	ctx := context.Background()
	...
	fixture.model.streamFn = func(_ context.Context, _ ModelRequest, _ func(token string) error) (ModelResponse, error) {
		...
		case 2:
			return ModelResponse{ToolCalls: []ModelToolCall{{
				ID:   "bootstrap-setup-bounded-first-wave",
				Name: "bootstrap.setup.persist",
				Tier: "tier1",
			}}}, nil
		default:
	...
	var childTaskID uuid.UUID
	for _, task := range tasks {
		if task.Title == "Define the first execution slice" {
			childTaskID = task.ID
			break
		}
	}
	if childTaskID == uuid.Nil {
		t.Fatal("expected first-wave child task")
	}
	fixture.engine.taskTransitions = &failSpecificQueueTransitionService{
		base:       fixture.engine.taskTransitions,
		failTaskID: childTaskID,
	}
	...
	if jobs := countRunnableAgentTurnJobsForSession(t, ctx, fixture.pool, projectSession.ID); jobs != 1 {
		t.Fatalf("runnable bootstrap agent_turn jobs = %d, want 1 recoverable continuation after bounded-size queue failure", jobs)
	}
	bootstrapState := projectBootstrapStateFromMetadata(storedSession.Metadata)
	if bootstrapState.Status != projectBootstrapStatusActive {
		t.Fatalf("bootstrap status = %q, want %q", bootstrapState.Status, projectBootstrapStatusActive)
	...
	if bootstrapState.ValidationStatus != "" {
		t.Fatalf("bootstrap validation_status = %q, want cleared while recoverable continuation is active", bootstrapState.ValidationStatus)
	}
	if bootstrapState.ValidationFailureClass != "" || bootstrapState.ValidationFailureReason != "" {
		t.Fatalf("bootstrap validation failure detail = (%q, %q), want cleared while recoverable continuation is active", bootstrapState.ValidationFailureClass, bootstrapState.ValidationFailureReason)
	}
	if bootstrapState.AutoTurnCount != 1 {
		t.Fatalf("bootstrap auto_turn_count = %d, want 1 after recoverable continuation", bootstrapState.AutoTurnCount)
	}

	storedProject := mustGetProjectByID(t, ctx, fixture.pool, project.ID)
	if storedProject.Status != "active" {
		t.Fatalf("project status = %q, want active while recoverable bootstrap validation is retried", storedProject.Status)
	}
}
...
func TestTurnEngineIntegrationProjectBootstrapQueuesRecoveryAfterRecoverableFirstWaveValidationFailure(t *testing.T) {
	...
	fixture.model.streamFn = func(_ context.Context, _ ModelRequest, _ func(token string) error) (ModelResponse, error) {
		...
		case 2:
			return ModelResponse{ToolCalls: []ModelToolCall{{
				ID:   "bootstrap-setup-recoverable-first-wave",
				Name: "bootstrap.setup.persist",
				Tier: "tier1",
			}}}, nil
		default:
	...
	if jobs := countRunnableAgentTurnJobsForSession(t, ctx, fixture.pool, projectSession.ID); jobs != 1 {
		t.Fatalf("runnable bootstrap agent_turn jobs = %d, want 1 recoverable continuation (status=%q current_turn_id=%v auto_turn_count=%d validation_class=%q continuation_messages=%d)", jobs, storedSession.Status, storedSession.CurrentTurnID, projectBootstrapStateFromMetadata(storedSession.Metadata).AutoTurnCount, projectBootstrapStateFromMetadata(storedSession.Metadata).ValidationFailureClass, continuationMessages)
	}
	if storedSession.CurrentTurnID != nil {
		t.Fatalf("project session current_turn_id = %v, want nil while recovery turn is queued", storedSession.CurrentTurnID)
	}
	bootstrapState := projectBootstrapStateFromMetadata(storedSession.Metadata)
	if bootstrapState.Status != projectBootstrapStatusActive {
		t.Fatalf("bootstrap status = %q, want %q (failure_class=%q failure_reason=%q validation_class=%q continuation_messages=%d)", bootstrapState.Status, projectBootstrapStatusActive, bootstrapState.FailureClass, bootstrapState.FailureReason, bootstrapState.ValidationFailureClass, continuationMessages)
	}
	if bootstrapState.ValidationFailureClass != "" || bootstrapState.ValidationFailureReason != "" {
		t.Fatalf("bootstrap validation failure detail = (%q, %q), want cleared while recoverable continuation is active", bootstrapState.ValidationFailureClass, bootstrapState.ValidationFailureReason)
	}
	if bootstrapState.AutoTurnCount != 1 {
		t.Fatalf("bootstrap auto_turn_count = %d, want 1 after recoverable validation continuation", bootstrapState.AutoTurnCount)
	}

	if continuationMessages != 2 {
		t.Fatalf("bootstrap continuation messages = %d, want 2 (initial follow-on plus recoverable validation retry)", continuationMessages)
	}
...
func TestTurnEngineIntegrationMalformedNoDecomposeChildKickoffPreflightBlocksBeforeModelCall(t *testing.T) {
	fixture := newIntegrationFixture(t)
	ctx := context.Background()
	...
	taskSession, _ := mustCreateTaskSession(t, ctx, fixture, childTask, "operator opened an async child session before the job was enqueued")
	authorType := "human_user"
	...
	if err := fixture.engine.handleUserMessage(ctx, taskSession.ID, kickoffMessage.ID, &fixture.agent.ID, 0, nil); err != nil {
		t.Fatalf("handleUserMessage malformed child kickoff: %v", err)
	}
	if modelCalls != 0 {
		t.Fatalf("model calls = %d, want 0 because malformed child kickoff must halt in preflight", modelCalls)
	}
	...
	if !strings.Contains(blockedReason, "explicitly forbids decomposition") {
		t.Fatalf("blocked reason = %q, want malformed no-decompose explanation", blockedReason)
	}
	...
	for _, message := range messages {
		if message.TurnID == nil || *message.TurnID != latestCompleted.ID || !strings.EqualFold(strings.TrimSpace(message.Role), "system") {
			continue
		}
		if strings.Contains(message.Content, "malformed child of") && strings.Contains(message.Content, "should not continue") {
			foundSystemMessage = true
			break
		}
	}
	if !foundSystemMessage {
		t.Fatal("missing malformed-child preflight system message")
	}
}

func TestTurnEngineIntegrationMalformedProceduralChildKickoffPreflightBlocksBeforeModelCall(t *testing.T) {
	fixture := newIntegrationFixture(t)
	ctx := context.Background()
	...
	taskSession, _ := mustCreateTaskSession(t, ctx, fixture, childTask, "operator opened an async procedural child session before the job was enqueued")
	authorType := "human_user"
	kickoffMessage, err := fixture.chatService.AppendMessage(ctx, chat.AppendMessageInput{
		SessionID:  taskSession.ID,
		AuthorType: &authorType,
	...
	if err := fixture.engine.handleUserMessage(ctx, taskSession.ID, kickoffMessage.ID, &fixture.agent.ID, 0, nil); err != nil {
		t.Fatalf("handleUserMessage malformed procedural child kickoff: %v", err)
	}
	if modelCalls != 0 {
		t.Fatalf("model calls = %d, want 0 because malformed procedural child kickoff must halt in preflight", modelCalls)
	}
	...
	for _, message := range messages {
		...
		if strings.Contains(message.Content, "malformed procedural child") && strings.Contains(message.Content, "should not continue") {
			foundSystemMessage = true
			break
		}
	}
...
func TestTurnEngineIntegrationMalformedDuplicateSharedFileChildKickoffPreflightBlocksBeforeModelCall(t *testing.T) {
	...
	taskSession, _ := mustCreateTaskSession(t, ctx, fixture, childTask, "operator opened an async duplicate shared-file child session before the job was enqueued")
	authorType := "human_user"
	kickoffMessage, err := fixture.chatService.AppendMessage(ctx, chat.AppendMessageInput{
		SessionID:  taskSession.ID,
		AuthorType: &authorType,
	...
	if err := fixture.engine.handleUserMessage(ctx, taskSession.ID, kickoffMessage.ID, &fixture.agent.ID, 0, nil); err != nil {
		t.Fatalf("handleUserMessage malformed duplicate shared-file child kickoff: %v", err)
	}
	if modelCalls != 0 {
		t.Fatalf("model calls = %d, want 0 because malformed duplicate shared-file child kickoff must halt in preflight", modelCalls)
	}
...
func TestTurnEngineIntegrationDuplicateSameDeliverableOlderReviewTaskPreflightBlocksBeforeModelCall(t *testing.T) {
	fixture := newIntegrationFixture(t)
	ctx := context.Background()
	...
	olderReviewDescription := "Repair planning/sambot-prompts/test-conversations-technical.md by rewriting the six technical conversations after rejected proof."
	olderReviewTask, err := repo.NewProjectTaskRepo(fixture.pool).Create(ctx, repo.ProjectTask{
		OrganizationID:  fixture.org.ID,
		ProjectID:       project.ID,
		Title:           "Repair planning/sambot-prompts/test-conversations-technical.md — fix rejected proof (6 technical conversations)",
		Description:     &olderReviewDescription,
		WorkStatus:      "review",
		FlowTemplateID:  &flowTemplate.ID,
		CreatedByType:   "human_user",
		CreatedByID:     &fixture.user.ID,
	...
	if err != nil {
		t.Fatalf("Create older review task: %v", err)
	}

	newerReviewDescription := "Rewrite planning/sambot-prompts/test-conversations-technical.md — 6 technical test conversations (replaces rejected OC-12038)."
	newerReviewTask, err := repo.NewProjectTaskRepo(fixture.pool).Create(ctx, repo.ProjectTask{
		OrganizationID:  fixture.org.ID,
		ProjectID:       project.ID,
		Title:           "Rewrite planning/sambot-prompts/test-conversations-technical.md — 6 technical test conversations (replaces rejected OC-12038)",
		Description:     &newerReviewDescription,
		WorkStatus:      "review",
		FlowTemplateID:  &flowTemplate.ID,
		CreatedByType:   "human_user",
		CreatedByID:     &fixture.user.ID,
	...
	if err != nil {
		t.Fatalf("Create newer review task: %v", err)
	}

	taskSession, _ := mustCreateTaskSession(t, ctx, fixture, olderReviewTask, "operator reopened older same-deliverable review lane while newer review lane was already active")
	authorType := "human_user"
	kickoffMessage, err := fixture.chatService.AppendMessage(ctx, chat.AppendMessageInput{
		SessionID:  taskSession.ID,
		AuthorType: &authorType,
		AuthorID:   &fixture.user.ID,
		Role:       "user",
		Content:    buildTaskQueueKickoffMessageForTest(olderReviewTask),
		Metadata: mustJSON(t, map[string]any{
			"source": "task_queue_processor",
		}),
	})
	...
	if err := fixture.engine.handleUserMessage(ctx, taskSession.ID, kickoffMessage.ID, &fixture.agent.ID, 0, nil); err != nil {
		t.Fatalf("handleUserMessage duplicate same-deliverable older review kickoff: %v", err)
	}
	if modelCalls != 0 {
		t.Fatalf("model calls = %d, want 0 because duplicate same-deliverable older review kickoff must halt in preflight", modelCalls)
	}

	refreshedTask, err := repo.NewProjectTaskRepo(fixture.pool).GetByID(ctx, olderReviewTask.ID)
	if err != nil {
		t.Fatalf("GetByID refreshed task: %v", err)
	}
	if refreshedTask.WorkStatus != "blocked" {
	...
	blockedReasons, err := repo.NewProjectTaskEventRepo(fixture.pool).LatestBlockedReasonsByTask(ctx, []uuid.UUID{olderReviewTask.ID})
	if err != nil {
		t.Fatalf("LatestBlockedReasonsByTask: %v", err)
	}
	blockedReason := blockedReasons[olderReviewTask.ID]
	if !strings.Contains(blockedReason, buildTaskLabel(newerReviewTask)) || !strings.Contains(blockedReason, "higher-precedence task lane") {
		t.Fatalf("blocked reason = %q, want newer same-deliverable review explanation", blockedReason)
	}
}
...
func TestTurnEngineIntegrationMalformedConflictingDeliverableChildKickoffPreflightBlocksBeforeModelCall(t *testing.T) {
	...
	taskSession, _ := mustCreateTaskSession(t, ctx, fixture, childTask, "operator opened an async conflicting-deliverable child session before the job was enqueued")
	authorType := "human_user"
	kickoffMessage, err := fixture.chatService.AppendMessage(ctx, chat.AppendMessageInput{
		SessionID:  taskSession.ID,
		AuthorType: &authorType,
	...
	if err := fixture.engine.handleUserMessage(ctx, taskSession.ID, kickoffMessage.ID, &fixture.agent.ID, 0, nil); err != nil {
		t.Fatalf("handleUserMessage malformed conflicting-deliverable child kickoff: %v", err)
	}
	if modelCalls != 0 {
		t.Fatalf("model calls = %d, want 0 because malformed conflicting-deliverable child kickoff must halt in preflight", modelCalls)
	}
	...
	if blockedReason := blockedReasons[childTask.ID]; !strings.Contains(blockedReason, "visible task brief still targets") {
		t.Fatalf("blocked reason = %q, want conflicting deliverable explanation", blockedReason)
	}
}

func TestTurnEngineIntegrationMissingSharedParentFileChildKickoffPreflightBlocksBeforeModelCall(t *testing.T) {
	fixture := newIntegrationFixture(t)
	ctx := context.Background()
	...
	parentDescription := "Write the file `planning/sambot-prompts/test-conversations-level3.md` containing exactly 3 deeply technical SamBot conversations."
	parentTask, err := repo.NewProjectTaskRepo(fixture.pool).Create(ctx, repo.ProjectTask{
		OrganizationID: fixture.org.ID,
		ProjectID:      project.ID,
	...
	flowTemplate := mustCreateExecutionFlowTemplate(t, ctx, fixture.pool, fixture.org.ID, project.ID, fixture.user.ID)
	childDescription := "Write deeply technical test conversation 1: failure recovery)"
	childTask, err := repo.NewProjectTaskRepo(fixture.pool).Create(ctx, repo.ProjectTask{
		OrganizationID:  fixture.org.ID,
		ProjectID:       project.ID,
		Title:           "Write deeply technical test conversation 1: failure recovery)",
		Description:     &childDescription,
		WorkStatus:      "in_progress",
		FlowTemplateID:  &flowTemplate.ID,
		CreatedByType:   "human_user",
	...
	taskSession, _ := mustCreateTaskSession(t, ctx, fixture, childTask, "operator opened an async missing-shared-parent child session before the job was enqueued")
	authorType := "human_user"
	kickoffMessage, err := fixture.chatService.AppendMessage(ctx, chat.AppendMessageInput{
		SessionID:  taskSession.ID,
		AuthorType: &authorType,
	...
	if err := fixture.engine.handleUserMessage(ctx, taskSession.ID, kickoffMessage.ID, &fixture.agent.ID, 0, nil); err != nil {
		t.Fatalf("handleUserMessage missing shared-parent child kickoff: %v", err)
	}
	if modelCalls != 0 {
		t.Fatalf("model calls = %d, want 0 because missing shared-parent child kickoff must halt in preflight", modelCalls)
	}
	...
	if blockedReason := blockedReasons[childTask.ID]; !strings.Contains(blockedReason, "missing shared parent deliverable") {
		t.Fatalf("blocked reason = %q, want missing shared parent deliverable explanation", blockedReason)
	}
}
...
func TestTurnEngineIntegrationProjectBootstrapFailsWhenAllFirstWaveTasksAreUnassigned(t *testing.T) {
	fixture := newIntegrationFixture(t)
	ctx := context.Background()
	...
	fixture.model.streamFn = func(_ context.Context, _ ModelRequest, _ func(token string) error) (ModelResponse, error) {
		...
		case 2:
			return ModelResponse{ToolCalls: []ModelToolCall{{
				ID:   "bootstrap-setup-unassigned-first-wave",
				Name: "bootstrap.setup.persist",
				Tier: "tier1",
			}}}, nil
		default:
	...
	fixture.dispatcher.tier1Fn = func(ctx context.Context, call ToolCall) (ToolResult, error) {
		template := mustCreateExecutionFlowTemplate(t, ctx, fixture.pool, fixture.org.ID, project.ID, fixture.user.ID)
		description := "Research bounded speaker-opportunity programs and capture the initial findings."
		taskRecord, err := repo.NewProjectTaskRepo(fixture.pool).Create(ctx, repo.ProjectTask{
			OrganizationID: fixture.org.ID,
			ProjectID:      project.ID,
			Title:          "Research bounded speaker opportunity programs",
			Description:    &description,
			WorkStatus:     "draft",
			FlowTemplateID: &template.ID,
			CreatedByType:  "agent",
	...
	if !strings.Contains(bootstrapState.ValidationFailureReason, "has no assigned agent") {
		t.Fatalf("bootstrap validation_failure_reason = %q, want missing assignment guidance", bootstrapState.ValidationFailureReason)
	}
...
func TestTurnEngineIntegrationSeededBootstrapRestartAckOnlyWithNoProgressFailsImmediately(t *testing.T) {
	...
	record := projectAutomaticFailureRecord{
		...
		LastCheckpoint:           projectBootstrapCheckpointFirstWaveSelected,
		LastSuccessfulCheckpoint: projectBootstrapCheckpointFirstWaveSelected,
		FailureReason:            buildProjectBootstrapFirstWaveAssignmentFailureReason(taskRecord),
		SetupPersisted:           true,
		RecordedAt:               time.Now().UTC(),
	}
	if err := fixture.engine.maybeRestartArchivedBootstrapProject(ctx, archivedProject, record, true); err != nil {
	...
	if err := fixture.engine.handleUserMessage(ctx, payload.SessionID, payload.MessageID, payload.AgentID, payload.RetryCount, &jobID); err != nil {
		t.Fatalf("handleUserMessage restart acknowledgement turn: %v", err)
	}
	...
	if latestTurn := latestCompletedTurn(turns); latestTurn != nil {
		if err := fixture.engine.HandleTurnCompletedEvent(ctx, eventbus.DomainEvent{
			OrganizationID: fixture.org.ID,
			EventType:      "chat.turn.completed",
			Payload:        mustJSON(t, map[string]any{"session_id": restartedSession.ID.String(), "turn_id": latestTurn.ID.String()}),
		}); err != nil {
			t.Fatalf("HandleTurnCompletedEvent restart acknowledgement turn: %v", err)
		}
	}
	...
	if !strings.Contains(strings.ToLower(bootstrapState.FailureReason), "acknowledgement only") {
		t.Fatalf("bootstrap failure_reason = %q, want acknowledgement-only failure", bootstrapState.FailureReason)
	}
	if jobs := countRunnableAgentTurnJobsForSession(t, ctx, fixture.pool, restartedSession.ID); jobs != 0 {
		t.Fatalf("runnable bootstrap jobs = %d, want 0 after ack-only restart failure", jobs)
	}
}
...
func TestTurnEngineIntegrationProjectBootstrapFailsValidationForBroadParentOnlySetup(t *testing.T) {
	...
	if jobs := countRunnableAgentTurnJobsForSession(t, ctx, fixture.pool, projectSession.ID); jobs != 1 {
		t.Fatalf("runnable bootstrap agent_turn jobs = %d, want 1 recoverable continuation after parent-only validation failure", jobs)
	}
	...
	if bootstrapState.FailureClass != "" || bootstrapState.FailureReason != "" {
		t.Fatalf("bootstrap failure detail = (%q, %q), want cleared while recoverable continuation is active", bootstrapState.FailureClass, bootstrapState.FailureReason)
	}
	if bootstrapState.AutoTurnCount != 1 {
		t.Fatalf("bootstrap auto_turn_count = %d, want 1 after recoverable continuation", bootstrapState.AutoTurnCount)
	}
...
func TestTurnEngineIntegrationRecoveredLeakedBootstrapRetryUsesValidationContinuation(t *testing.T) {
	fixture := newIntegrationFixture(t)
	ctx := context.Background()
	...
	`, jobID, "task exceeds bounded size policy (estimated 35 minutes > 30 minute limit): split the work into smaller reviewable tasks before queueing"); err != nil {
		t.Fatalf("Update job_queue retry state: %v", err)
	}
	...
	recovered, err := fixture.engine.recoverRetriedAgentTurnLeak(ctx, sessionSnapshot, handoff.ID, lori.ID, 0, &jobID, activeTurn)
	if err != nil {
		t.Fatalf("recoverRetriedAgentTurnLeak: %v", err)
	}
	if !recovered {
		t.Fatal("expected stale bootstrap retry to recover into validation continuation")
	}
	...
	if queuedMessageID == handoff.ID {
		t.Fatal("expected recovered retry to enqueue a new continuation message, not the original handoff")
	}
}

func TestTurnEngineIntegrationUpdateProjectBootstrapStateClearsBootstrapAutomaticFailureWhenActiveHealthy(t *testing.T) {
	fixture := newIntegrationFixture(t)
	ctx := context.Background()
	...
	project := mustCreateBootstrapProject(t, ctx, fixture)
	projectSession := mustCreateProjectSession(t, ctx, fixture, project.ID, fixture.agent.ID, lori.ID)
	handoff := mustAppendProjectBootstrapHandoff(t, ctx, fixture, projectSession.ID, fixture.agent.ID, "Frank handoff: continue the active bootstrap from persisted state.")

	recordedAt := fixture.engine.now().UTC()
	project.Settings, _ = projectfailure.Apply(project.Settings, projectfailure.State{
	...
	if _, err := repo.NewProjectRepo(fixture.pool).Update(ctx, project); err != nil {
		t.Fatalf("Update project with stale automatic failure: %v", err)
	}
	...
	if err := fixture.engine.updateProjectBootstrapState(ctx, sessionSnapshot, projectBootstrapState{
		...
		t.Fatalf("updateProjectBootstrapState active healthy: %v", err)
	}

	storedProject := mustGetProjectByID(t, ctx, fixture.pool, project.ID)
	if failureState := projectfailure.Parse(storedProject.Settings); failureState.Action != "" || failureState.Source != "" {
		t.Fatalf("automatic failure state = %+v, want cleared after healthy active bootstrap progress", failureState)
	}
	projectBootstrapState := mustProjectBootstrapProjectState(t, storedProject)
	if projectBootstrapState.Status != projectBootstrapStatusActive {
		t.Fatalf("project bootstrap status = %q, want %q", projectBootstrapState.Status, projectBootstrapStatusActive)
	}
}

func TestTurnEngineIntegrationUpdateProjectBootstrapStateClearsBootstrapAutomaticFailureWhenCompleted(t *testing.T) {
	fixture := newIntegrationFixture(t)
	ctx := context.Background()
	...
	project := mustCreateBootstrapProject(t, ctx, fixture)
	projectSession := mustCreateProjectSession(t, ctx, fixture, project.ID, fixture.agent.ID, lori.ID)
	handoff := mustAppendProjectBootstrapHandoff(t, ctx, fixture, projectSession.ID, fixture.agent.ID, "Frank handoff: bootstrap already completed.")
	...
	storedProject := mustGetProjectByID(t, ctx, fixture.pool, project.ID)
	if failureState := projectfailure.Parse(storedProject.Settings); failureState.Action != "" || failureState.Source != "" {
		t.Fatalf("automatic failure state = %+v, want cleared after completed bootstrap", failureState)
	}
	projectBootstrapState := mustProjectBootstrapProjectState(t, storedProject)
	if projectBootstrapState.Status != projectBootstrapStatusCompleted {
		t.Fatalf("project bootstrap status = %q, want %q", projectBootstrapState.Status, projectBootstrapStatusCompleted)
	}
}
...
func TestTurnEngineIntegrationRecoveredProjectTaskStaleInboundTurnWithoutRunKeepsLiveInvocation(t *testing.T) {
	fixture := newIntegrationFixture(t)
	ctx := context.Background()
	...
	taskRecord, err := repo.NewProjectTaskRepo(fixture.pool).Create(ctx, repo.ProjectTask{
		OrganizationID:  fixture.org.ID,
		ProjectID:       project.ID,
		Title:           "Keep live task inbound turn with invocation",
		WorkStatus:      "review",
		BlocksScope:     "task",
		FlowTemplateID:  &template.ID,
		CreatedByType:   "agent",
	...
	message, err := fixture.chatService.AppendMessage(ctx, chat.AppendMessageInput{
		...
		AuthorID:   &fixture.agent.ID,
		Role:       "user",
		Content:    "Review only. Inspect the deliverable and decide.",
	})
	if err != nil {
		t.Fatalf("AppendMessage task handoff: %v", err)
	}

	liveTurn, _, err := fixture.engine.turns.CreateForMessageAttempt(ctx, session.ID, fixture.agent.ID, message.ID, 0)
	if err != nil {
		t.Fatalf("CreateForMessageAttempt live turn: %v", err)
	}
	startedTurn, shouldRun, err := fixture.engine.startInboundMessageTurn(ctx, liveTurn)
	if err != nil {
		t.Fatalf("startInboundMessageTurn live turn: %v", err)
	}
	if !shouldRun {
		t.Fatal("expected live inbound turn to start initially")
	}

	provider, err := repo.NewModelProviderRepo(fixture.pool).Create(ctx, repo.ModelProvider{
		Slug:        "keep-live-task-inbound-turn-provider",
		DisplayName: "Keep Live Task Inbound Turn Provider",
	...
	if _, err := repo.NewModelInvocationRepo(fixture.pool).Create(ctx, repo.ModelInvocation{
		OrganizationID:    fixture.org.ID,
	...
	if err := fixture.engine.HandleTurnJob(ctx, claimedJob); err != nil {
		t.Fatalf("HandleTurnJob live-invocation retry: %v", err)
	}

	storedTurn, err := fixture.chatService.GetTurn(ctx, startedTurn.ID)
	if err != nil {
		t.Fatalf("GetTurn live turn: %v", err)
	}
	if storedTurn.Status != "in_progress" {
		t.Fatalf("live turn status = %q, want in_progress", storedTurn.Status)
	}
	...
	if storedSession.CurrentTurnID == nil || *storedSession.CurrentTurnID != startedTurn.ID {
		t.Fatalf("current_turn_id = %v, want live turn %s", storedSession.CurrentTurnID, startedTurn.ID)
	}
	...
	if pendingJobs != 0 {
		t.Fatalf("pending task agent_turn jobs = %d, want 0 when live invocation exists", pendingJobs)
	}
}
...
func TestTurnEngineIntegrationHandleRecoverableBootstrapTurnJobFailureHandlesBoundedSizeError(t *testing.T) {
	...
	project := mustCreateBootstrapProject(t, ctx, fixture)
	projectSession := mustCreateProjectSession(t, ctx, fixture, project.ID, fixture.agent.ID, lori.ID)
	handoff := mustAppendProjectBootstrapHandoff(t, ctx, fixture, projectSession.ID, fixture.agent.ID, "Frank handoff: recover bounded bootstrap tasks into a fresh continuation.")
	pmAgent := mustCreateBootstrapPMAgent(t, ctx, fixture.pool, fixture.org.ID)
	template := mustCreateExecutionFlowTemplate(t, ctx, fixture.pool, fixture.org.ID, project.ID, fixture.user.ID)
	...
	handled, err := fixture.engine.handleRecoverableBootstrapTurnJobFailure(ctx, AgentTurnPayload{
		...
		MessageID: handoff.ID,
		AgentID:   &lori.ID,
	}, &jobID, errors.New("task exceeds bounded size policy (estimated 55 minutes > 30 minute limit): split the work into smaller reviewable tasks before queueing"))
	if err != nil {
		t.Fatalf("handleRecoverableBootstrapTurnJobFailure: %v", err)
	}
	if !handled {
		t.Fatal("expected bounded size bootstrap failure to recover into fresh continuation")
	}
	...
	if storedTurn.Status != "failed" {
		t.Fatalf("active turn status = %q, want failed after bounded size recovery", storedTurn.Status)
	}
	...
	if queuedMessageID == handoff.ID {
		t.Fatal("expected bounded size recovery to enqueue a new continuation message")
	}
}
...
func TestTurnEngineIntegrationProjectExecutionContinuationAutoQueuesRunnableDraftAfterNarrativeOnlyTurn(t *testing.T) {
	fixture := newIntegrationFixture(t)
	ctx := context.Background()
	...
	messageRepo := repo.NewChatMessageRepo(fixture.pool)
	continuationMetadata := mustJSON(t, map[string]any{
	...
	if err != nil {
		t.Fatalf("create continuation message: %v", err)
	}
	...
	if _, err := messageRepo.Create(ctx, repo.ChatMessage{
		...
		TurnID:    &turn.ID,
		Role:      "assistant",
		Content:   "The remaining draft tasks are auxiliary and are not blockers for the completed project.",
		Status:    "final",
	}); err != nil {
		t.Fatalf("create assistant narrative: %v", err)
	}
	...
	if err := fixture.engine.HandleTurnCompletedEvent(ctx, eventbus.DomainEvent{
		...
		EventType:      "chat.turn.completed",
		Payload: mustJSON(t, map[string]any{
			"session_id": projectSession.ID.String(),
			"turn_id":    turn.ID.String(),
		}),
	}); err != nil {
		t.Fatalf("HandleTurnCompletedEvent project continuation: %v", err)
	}

	updatedDraft, err := taskRepo.GetByID(ctx, draftTask.ID)
	if err != nil {
	...
	messages, err := messageRepo.ListBySession(ctx, projectSession.ID)
	if err != nil {
		t.Fatalf("ListBySession project messages: %v", err)
	}
	var sawAutoQueue bool
	for _, msg := range messages {
		if !strings.EqualFold(strings.TrimSpace(msg.Role), "system") {
			continue
		}
		if strings.Contains(msg.Content, "auto-queued task 19") {
			sawAutoQueue = true
			break
		}
	}
	if !sawAutoQueue {
		t.Fatal("expected system message noting auto-queued runnable draft task")
	}
}
...
func TestTurnEngineIntegrationProjectExecutionContinuationIgnoresTerminalTaskSessionsAndExecutions(t *testing.T) {
	fixture := newIntegrationFixture(t)
	ctx := context.Background()
	...
	project := mustCreateBootstrapProject(t, ctx, fixture)
	projectSession := mustCreateProjectSession(t, ctx, fixture, project.ID, fixture.agent.ID, lori.ID)
	handoff := mustAppendProjectBootstrapHandoff(t, ctx, fixture, projectSession.ID, fixture.agent.ID, "Frank handoff: project bootstrap already completed.")
	...
	completed := createTask(10, "Completed first-wave task", "done", workerAgent.ID)
	_ = createTask(11, "Draft follow-on task A", "draft", workerAgent.ID)
	_ = createTask(12, "Draft follow-on task B", "draft", workerAgent.ID)

	taskSession, _ := mustCreateTaskSession(t, ctx, fixture, completed, "stale terminal lane still closing down")
	execution, err := repo.NewFlowNodeExecutionRepo(fixture.pool).Create(ctx, repo.FlowNodeExecution{
		TaskID:      completed.ID,
	...
	if err != nil {
		t.Fatalf("Create terminal-task flow execution: %v", err)
	}
	if _, err := fixture.pool.Exec(ctx, `
		UPDATE flow_node_execution
		SET metadata = jsonb_set(COALESCE(metadata, '{}'::jsonb), '{live_turn_id}', to_jsonb($2::text), true)
		WHERE id = $1
	`, execution.ID, uuid.NewString()); err != nil {
		t.Fatalf("seed execution metadata: %v", err)
	}
	...
	if err := fixture.engine.HandleTaskStatusChangedEvent(ctx, eventbus.DomainEvent{
		...
		t.Fatalf("HandleTaskStatusChangedEvent terminal-lane completion: %v", err)
	}
...
func TestTurnEngineIntegrationProjectExecutionContinuationAfterTerminalTaskTurn(t *testing.T) {
	fixture := newIntegrationFixture(t)
	ctx := context.Background()
	...
	template := mustCreateReviewFlowTemplate(t, ctx, fixture.pool, fixture.org.ID)
	nodes, err := repo.NewFlowNodeRepo(fixture.pool).ListByTemplate(ctx, template.ID)
	if err != nil {
		t.Fatalf("ListByTemplate review flow: %v", err)
	}
	if len(nodes) < 2 {
		t.Fatalf("review flow nodes = %d, want at least 2", len(nodes))
	}
	reviewNodeID := nodes[1].ID
	taskRepo := repo.NewProjectTaskRepo(fixture.pool)
	createTask := func(number int, title, status string, assignedID uuid.UUID, currentNodeID *uuid.UUID) repo.ProjectTask {
		description := fmt.Sprintf("Task %d description.", number)
		input := repo.ProjectTask{
			OrganizationID:    fixture.org.ID,
			ProjectID:         project.ID,
	...
	_ = createTask(9, "Completed first-wave task A", "done", workerAgent.ID, nil)
	completed := createTask(10, "Completed review task", "done", reviewerAgent.ID, &reviewNodeID)
	_ = createTask(11, "Draft follow-on task A", "draft", workerAgent.ID, nil)
	_ = createTask(12, "Draft follow-on task B", "draft", workerAgent.ID, nil)

	taskSession, userMessage := mustCreateTaskSession(t, ctx, fixture, completed, "review and approve the deliverable")
	execution, err := repo.NewFlowNodeExecutionRepo(fixture.pool).Create(ctx, repo.FlowNodeExecution{
		TaskID:      completed.ID,
	...
	completedTurn, err := repo.NewChatTurnRepo(fixture.pool).Create(ctx, repo.ChatTurn{
		SessionID:        taskSession.ID,
		TurnNumber:       1,
		RespondingType:   "agent",
		RespondingID:     reviewerAgent.ID,
		Status:           "completed",
		TriggerMessageID: &userMessage.ID,
	})
	if err != nil {
		t.Fatalf("Create completed review turn: %v", err)
	}
	...
	if err := fixture.engine.HandleTurnCompletedEvent(ctx, eventbus.DomainEvent{
		...
		Payload: mustJSON(t, map[string]any{
			...
			"turn_id":    completedTurn.ID.String(),
		}),
	}); err != nil {
		t.Fatalf("HandleTurnCompletedEvent terminal task turn: %v", err)
	}
...
func TestTurnEngineIntegrationBootstrapProgressPrefersAssignedLeavesForFirstWaveValidation(t *testing.T) {
	fixture := newIntegrationFixture(t)
	ctx := context.Background()
	...
	assignedDescription := "Produce the first reviewable SAM.blog homepage implementation slice."
	assignedTask, err := repo.NewProjectTaskRepo(fixture.pool).Create(ctx, repo.ProjectTask{
		OrganizationID:  fixture.org.ID,
		ProjectID:       project.ID,
		Title:           "Build first homepage slice",
	...
	unassignedDescription := "Draft the second follow-on implementation slice after the first reviewable page is in motion."
	if _, err := repo.NewProjectTaskRepo(fixture.pool).Create(ctx, repo.ProjectTask{
		OrganizationID: fixture.org.ID,
		ProjectID:      project.ID,
		Title:          "Plan second homepage slice",
		...
		t.Fatalf("Create unassigned planned task: %v", err)
	}
...
func TestTurnEngineIntegrationKickoffBlockedSessionCreateStopsAndBackfillsProjectHandoff(t *testing.T) {
	fixture := newIntegrationFixture(t)
	ctx := context.Background()
	...
	projectSlug := "sam-blog-kickoff-blocked-session"
	dispatched := make([]string, 0, 2)
	...
	fixture.dispatcher.tier1Fn = func(_ context.Context, call ToolCall) (ToolResult, error) {
		...
		createdProjectID = updatedProject.ID
		return ToolResult{
			ToolCallID: call.ID,
			Name:       call.Name,
			Output: map[string]any{
	...
	fixture.model.streamFn = func(_ context.Context, _ ModelRequest, _ func(token string) error) (ModelResponse, error) {
		...
		case 2:
			if createdProjectID == uuid.Nil {
				t.Fatal("createdProjectID missing before blocked session.create")
			}
			return ModelResponse{ToolCalls: []ModelToolCall{
				{ID: "session-1", Name: "session.create", Tier: "tier1", Arguments: map[string]any{
					"scope_type": "project",
					"scope_id":   createdProjectID.String(),
					"mode":       "async",
				}},
			}}, nil
		default:
	...
	orgMessages, err := repo.NewChatMessageRepo(fixture.pool).ListBySession(ctx, fixture.session.ID)
	if err != nil {
		t.Fatalf("ListBySession org messages: %v", err)
	}
	hasBlockedFollowOn := false
	for _, message := range orgMessages {
		if strings.EqualFold(strings.TrimSpace(message.Role), "tool_result") &&
			message.ToolCallID != nil &&
			*message.ToolCallID == "session-1" &&
			strings.Contains(strings.ToLower(message.Content), "handoff-only") {
			hasBlockedFollowOn = true
			break
		}
	...
	if !hasBlockedFollowOn {
		t.Fatal("missing blocked follow-on session.create tool_result")
	}
	...
	if err := fixture.pool.QueryRow(ctx, `
		...
		WHERE organization_id = $1
		  ...
		  AND mode = 'async'
		  AND status = 'active'
	`, fixture.org.ID, createdProjectID).Scan(&projectSessionID); err != nil {
		t.Fatalf("lookup project session: %v", err)
	}

	projectMessages, err := repo.NewChatMessageRepo(fixture.pool).ListBySession(ctx, projectSessionID)
	if err != nil {
	...
	if !strings.Contains(strings.ToLower(handoff.Content), "originating user request:") ||
		!strings.Contains(strings.ToLower(handoff.Content), "imported legacy posts") ||
	...
	if jobs := countRunnableAgentTurnJobsForSession(t, ctx, fixture.pool, projectSessionID); jobs != 1 {
		t.Fatalf("runnable project kickoff jobs = %d, want 1", jobs)
	}
	_, payload := dequeueNextAgentTurnForSession(t, ctx, fixture.pool, projectSessionID)
...
func TestTurnEngineIntegrationRecoverCancelledBootstrapSessionsRequeuesWhenContinuationMessageHasNoJob(t *testing.T) {
	fixture := newIntegrationFixture(t)
	ctx := context.Background()
	...
	if _, err := fixture.engine.appendProjectBootstrapContinuationMessage(ctx, projectSession.ID, lori.ID, handoff.ID.String(), 1); err != nil {
		t.Fatalf("appendProjectBootstrapContinuationMessage: %v", err)
	}
...
func TestTurnEngineHandleProjectResumedEventRequeuesPendingTurnsWithoutJobs(t *testing.T) {
	fixture := newIntegrationFixture(t)
	ctx := context.Background()

	project, err := repo.NewProjectRepo(fixture.pool).Create(ctx, repo.Project{
		OrganizationID: fixture.org.ID,
		Slug:           "resume-requeue-project-" + uuid.NewString()[:8],
		DisplayName:    "Resume Requeue Project",
		DeliveryMode:   "gated",
		Status:         "active",
		CreatedByType:  "system",
		CreatedByID:    uuid.Nil,
	...
	description := "Resume pending work after the project is unpaused."
	taskRecord, err := repo.NewProjectTaskRepo(fixture.pool).Create(ctx, repo.ProjectTask{
		OrganizationID: fixture.org.ID,
		ProjectID:      project.ID,
		Title:          "Resume pending turn",
		Description:    &description,
		WorkStatus:     "draft",
		BlocksScope:    "task",
		CreatedByType:  "system",
	...
	if _, err := repo.NewChatSessionRepo(fixture.pool).UpdateCurrentTurn(ctx, session.ID, &turn.ID); err != nil {
...
func TestTurnEngineIntegrationExplicitDeliverableWriteRecordsCanonicalCommitBeforeReview(t *testing.T) {
	fixture := newIntegrationFixture(t)
	ctx := context.Background()
	t.Setenv("OTTERCAMP_DATA_DIR", t.TempDir())
	...
	project := mustCreateProject(t, ctx, fixture.pool, fixture.org.ID, fixture.user.ID)
	mustAssignProjectPM(t, ctx, fixture.pool, project.ID, fixture.agent.ID, fixture.user.ID)
	assigned := fixture.agent.ID
	createdBy := fixture.user.ID
	flowTemplate := mustCreateExecutionFlowTemplate(t, ctx, fixture.pool, fixture.org.ID, project.ID, fixture.user.ID)
	description := "Define validation scope for the current project. Output: validation_scope.md with scenario list, SLAs, and decision criteria."
	taskRecord, err := repo.NewProjectTaskRepo(fixture.pool).Create(ctx, repo.ProjectTask{
		OrganizationID:  fixture.org.ID,
		ProjectID:       project.ID,
	...
	outputRel := "validation_scope.md"
	outputAbs := filepath.Join(workspaceRoot, outputRel)
	if err := os.WriteFile(outputAbs, []byte("# Validation Scope\n\nSubstantive content.\n"), 0o644); err != nil {
		t.Fatalf("write output file: %v", err)
	}
	...
	userMessage, err := fixture.chatService.AppendMessage(ctx, chat.AppendMessageInput{
		...
		AuthorID:   &fixture.user.ID,
		Role:       "user",
		Content:    "finish the current work and write the deliverable",
	})
	if err != nil {
	...
	fileResult, err := fixture.chatService.AppendMessage(ctx, chat.AppendMessageInput{
		...
		TurnID:    &turn.ID,
		Role:      "tool_result",
		Content:   string(mustJSON(t, map[string]any{"tool_name": "file.write", "output": map[string]any{"path": outputRel, "byte_size": 39, "created": true}})),
	})
	if err != nil {
	...
	executions, err := repo.NewFlowNodeExecutionRepo(fixture.pool).ListByTask(ctx, taskRecord.ID)
	if err != nil {
	...
	statusOut, err := exec.CommandContext(ctx, "git", "-C", workspaceRoot, "status", "--porcelain").CombinedOutput()
	if err != nil {
		t.Fatalf("git status: %v (%s)", err, strings.TrimSpace(string(statusOut)))
	}
	if strings.TrimSpace(string(statusOut)) != "" {
		t.Fatalf("workspace not clean after canonical work commit: %s", strings.TrimSpace(string(statusOut)))
	}
...
func TestBuildTaskReviewActionPromptIgnoresHistoricalInputArtifactOutsidePreferredDeliverableRootIntegration(t *testing.T) {
	fixture := newIntegrationFixture(t)
	ctx := context.Background()
	...
	taskRecord.Title = "Scrape all 35 technonymous.org blog posts listed in content/technonymous-index.json and save each as a clean Markdown file under content/posts/."
	taskRecord.Description = &description
	taskRecord.WorkStatus = "review"
	if _, err := repo.NewProjectTaskRepo(fixture.pool).Update(ctx, taskRecord); err != nil {
		t.Fatalf("Update task: %v", err)
...
func mustLatestAgentTurnPayloadForSession(t *testing.T, ctx context.Context, pool *pgxpool.Pool, sessionID uuid.UUID) AgentTurnPayload {
	t.Helper()
	...
	if err := pool.QueryRow(ctx, `
		...
		LIMIT 1
	`, AgentTurnJobType, sessionID.String()).Scan(&rawPayload); err != nil {
		t.Fatalf("load agent_turn job payload for session %s: %v", sessionID, err)
	}
