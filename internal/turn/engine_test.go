...
func TestListeningEvalSkippedForSyntheticProjectContinuationMessage(t *testing.T) {
	fixture := newUnitFixture(t, "async")
	fixture.session.ScopeType = "project"
	fixture.session.ScopeID = uuid.New()
	if _, err := fixture.messages.UpdateMetadata(context.Background(), fixture.userMessageID, json.RawMessage(`{
	...
	fixture.model.completeFn = func(context.Context, ModelRequest) (ModelResponse, error) {
		t.Fatal("listening eval should be skipped for synthetic project continuation messages")
		return ModelResponse{}, nil
	}
	fixture.model.streamFn = func(ctx context.Context, req ModelRequest, onChunk func(token string) error) (ModelResponse, error) {
		if err := onChunk("ok"); err != nil {
	...
	if assembledHistoryStart == nil {
		t.Fatal("HistoryStartID is nil, want synthetic project continuation message id")
	}
	if *assembledHistoryStart != fixture.userMessageID {
		t.Fatalf("HistoryStartID = %s, want %s", *assembledHistoryStart, fixture.userMessageID)
	}
}
...
func TestHandleTurnJobAsyncOrganizationRateLimitedPreflightDefersPastRetryCapWithoutTurn(t *testing.T) {
	...
	retryAfter := 4*time.Minute + 9*time.Second
	fixture.model.probeFn = func(context.Context, ModelRequest) error {
		return NewRateLimitedError(retryAfter, errors.New("all provider connections are rate limited"))
	}
...
func TestHandleCompletedReviewTurnWithoutDecisionRetriesWithRejectPromptWhileWorkspaceStillDirty(t *testing.T) {
	fixture := newUnitFixture(t, "async")
	base := time.Unix(1700002050, 0).UTC()
	fixture.engine.now = func() time.Time { return base }
	...
	agentID := fixture.chat.participants[0].ParticipantID
	turnID := createCompletedTurnWithAssistantMessage(t, fixture, agentID, "The workspace is still dirty, so approval cannot proceed yet.")
	...
	if !strings.Contains(retryPrompt.Content, "workspace is still dirty") {
		t.Fatalf("retry prompt = %q, want dirty-workspace reference", retryPrompt.Content)
	}
	if !strings.Contains(retryPrompt.Content, "decision=reject") {
		t.Fatalf("retry prompt = %q, want explicit reject instruction", retryPrompt.Content)
	}
	if strings.Contains(retryPrompt.Content, "decision=approve") {
		t.Fatalf("retry prompt = %q, want no approve instruction while workspace remains dirty", retryPrompt.Content)
	}
}
...
func TestIsActionableProjectDraftTaskSkipsProjectContinuationMetaDrafts(t *testing.T) {
	...
	if isActionableProjectDraftTask(task) {
		t.Fatal("expected next-phase shell draft to be excluded from actionable draft count")
	}
...
func TestHandleToolValidationResultsStopsAsyncTaskTurnAfterSecondIdenticalFileListNotFoundInSameTurn(t *testing.T) {
	fixture := newUnitFixture(t, "async")
	taskID := uuid.New()
	projectID := uuid.New()
	turnID := uuid.New()
	path := "scripts"
	attemptFingerprint := toolargs.AttemptFingerprint("file.list", map[string]any{"path": path, "recursive": true})
	...
	metadata, err := mergeTaskValidationGuardMetadata(json.RawMessage(`{"existing":"value"}`), taskValidationGuardState{
		InitialMessageID:   fixture.userMessageID.String(),
		Fingerprint:        "file.list:not_found",
		AttemptFingerprint: attemptFingerprint,
		ToolName:           "file.list",
		FailureClass:       "tool_validation",
		FailureCode:        "not_found",
	...
	calls := []ToolCall{{
		ID:        "list-1",
		Name:      "file.list",
		Arguments: map[string]any{"path": path, "recursive": true},
	}}
	results := []ToolResult{{
		ToolCallID: "list-1",
		Name:       "file.list",
	...
	if !fixture.messages.containsContentSubstring("not_found") {
		t.Fatal("expected repeated file.list not_found early-stop validation message")
	}
}

func TestHandleToolValidationResultsStopsAsyncTaskTurnAfterSecondIdenticalTaskGitCommitBlockedInSameTurn(t *testing.T) {
	fixture := newUnitFixture(t, "async")
	taskID := uuid.New()
	projectID := uuid.New()
	turnID := uuid.New()
	attemptFingerprint := toolargs.AttemptFingerprint("git.commit", map[string]any{"message": "checkpoint work"})
	...
	metadata, err := mergeTaskValidationGuardMetadata(json.RawMessage(`{"existing":"value"}`), taskValidationGuardState{
		InitialMessageID:   fixture.userMessageID.String(),
		Fingerprint:        "git.commit:task_git_commit_blocked",
		AttemptFingerprint: attemptFingerprint,
		ToolName:           "git.commit",
		FailureClass:       "tool_validation",
		FailureCode:        "task_git_commit_blocked",
		FailureReason:      "task_git_commit_blocked",
		Count:              1,
		BlockThreshold:     validationLoopBlockThreshold,
		Blocked:            false,
		FirstSeenAt:        time.Now().UTC().Format(time.RFC3339Nano),
	...
	if !fixture.messages.containsContentSubstring("task_git_commit_blocked") {
		t.Fatal("expected repeated git.commit blocked early-stop validation message")
	}
}
...
func TestHandleToolValidationResultsStopsAsyncTaskTurnAfterThirdPackageInstallAttemptWithInterleavedChecks(t *testing.T) {
	fixture := newUnitFixture(t, "async")
	taskID := uuid.New()
	projectID := uuid.New()
	turnID := uuid.New()
	...
	steps := []priorStep{
		{
			callID:   "pkg-prev-1",
			command:  "pip install pyyaml 2>&1 | tail -3",
			exitCode: 0,
		},
		{
			callID:   "check-prev-1",
			command:  "python3 -c \"import yaml; print('yaml OK')\"",
			exitCode: 1,
		},
		{
			callID:   "pkg-prev-2",
			command:  "pip3 install pyyaml 2>&1 | tail -3",
			exitCode: 0,
		},
		{
			callID:   "check-prev-2",
			command:  "python3 -c \"import yaml; print('yaml OK at', yaml.__file__)\"",
			exitCode: 1,
		},
	}
	for _, step := range steps {
		fixture.messages.create(repo.ChatMessage{
			...
			Role:      "assistant",
			Status:    "final",
			Content:   "Trying another environment check.",
			Metadata: mustMarshalJSON(t, map[string]any{
				"tool_calls": []map[string]any{{
					"id":   step.callID,
					"name": "cli_execute",
	...
	fixture.messages.create(repo.ChatMessage{
		...
		Role:      "assistant",
		Status:    "final",
		Content:   "Trying the install directly through python -m pip.",
		Metadata: mustMarshalJSON(t, map[string]any{
			"tool_calls": []map[string]any{{
				"id":   "pkg-1",
				"name": "cli_execute",
				"arguments": map[string]any{
					"command": "/usr/bin/python3 -m pip install --user pyyaml 2>&1 | tail -5",
				},
			}},
		}),
	})
	...
	failures, err := fixture.engine.collectAsyncTaskToolResultChurnFailures(context.Background(), rt, results)
	if err != nil {
	...
	if got := failures[0].FailureCode; got != duplicatePackageInstallChurnCode {
		t.Fatalf("failure code = %q, want %q", got, duplicatePackageInstallChurnCode)
	}
...
func TestMaybeBlockRepeatedReadOnlyDiscoveryCapTurnsTreatsReadOnlyCLIExecuteAsDiscovery(t *testing.T) {
	t.Parallel()
	...
	taskRepo.items[taskID] = repo.ProjectTask{
		...
		ProjectID:  projectID,
		TaskNumber: 15,
		Title:      "Validate pipeline stage execution and ordering",
		WorkStatus: "in_progress",
	}
	blocker := &fakeTaskTransitionService{repo: taskRepo}
	fixture.engine.taskTransitions = blocker
				...
				"tool_calls": []map[string]any{
					{
						...
						"name": "cli.execute",
						"arguments": map[string]any{
							"command": "cd task/15 && pwd && ls -l scripts && git diff -- scripts/validate-stage-execution.sh | head -n 40",
						},
					},
				},
			}),
		})
		for _, message := range []repo.ChatMessage{
			{
				SessionID:     fixture.session.ID,
				TurnID:        &turnID,
				Role:          "tool_result",
				Content:       string(mustMarshalJSON(t, map[string]any{"tool_name": "file.search", "output": map[string]any{"path": "scripts", "matches": []any{}}})),
				ContentFormat: "text",
				Status:        "final",
			},
			{
				...
				TurnID:        &turnID,
				Role:          "tool_result",
				Content:       string(mustMarshalJSON(t, map[string]any{"tool_name": "git.status", "output": map[string]any{"stdout": " M scripts/validate-stage-execution.sh"}})),
				ContentFormat: "text",
				Status:        "final",
			},
			{
				...
				TurnID:        &turnID,
				Role:          "tool_result",
				ToolCallID:    &cliToolCallID,
				Content:       string(mustMarshalJSON(t, map[string]any{"tool_name": "cli.execute", "output": map[string]any{"exit_code": 0, "stdout_inline": "scripts/validate-stage-execution.sh"}})),
				ContentFormat: "text",
				Status:        "final",
			},
		} {
			fixture.messages.create(message)
		}
	}
	if currentTurn == nil {
		t.Fatal("missing current turn")
	...
	if !blocked {
		t.Fatal("expected repeated read-only cli.execute discovery churn to block the task lane")
	}
	if len(blocker.calls) != 1 {
		t.Fatalf("blocked calls = %d, want 1", len(blocker.calls))
	}
	if !strings.Contains(blocker.calls[0].reason, "`cli.execute`") {
		t.Fatalf("block reason = %q, want cli.execute included", blocker.calls[0].reason)
	}
}

func TestMaybeBlockRepeatedReadOnlyDiscoveryCapTurnsIgnoresMutatingCLIExecute(t *testing.T) {
	t.Parallel()
	...
	for i := 1; i <= maxReadOnlyDiscoveryCapTurns; i++ {
				...
				"tool_calls": []map[string]any{
					{
						...
						"name": "cli.execute",
						"arguments": map[string]any{
							"command": "python3 scripts/validate-error-handling.sh",
						},
					},
				},
			}),
		...
		fixture.messages.create(repo.ChatMessage{
			...
			Role:          "tool_result",
			ToolCallID:    &cliToolCallID,
			Content:       string(mustMarshalJSON(t, map[string]any{"tool_name": "cli.execute", "output": map[string]any{"exit_code": 0, "stdout_inline": "ok"}})),
			ContentFormat: "text",
			Status:        "final",
		})
	}
	...
	if blocked {
		t.Fatal("expected mutating cli.execute history not to trigger read-only discovery block")
	}
	if len(blocker.calls) != 0 {
		t.Fatalf("blocked calls = %d, want 0", len(blocker.calls))
	}
...
func TestMaybeBlockRepeatedReadOnlyDiscoveryCapTurnsRetriesReviewLane(t *testing.T) {
	t.Parallel()
	...
	if !blocked {
		t.Fatal("expected repeated read-only discovery churn to stop the review lane")
	}
	if len(blocker.calls) != 0 {
...
func TestHandleTaskFileWriteWrongPathSkipsCrossArtifactFamilyRewrite(t *testing.T) {
	...
	taskID := uuid.New()
	turnID := uuid.New()
	description := "Implement the pipeline logger. Deliverable: src/pipeline_logger.py"
	...
	if got := stringValue(call.Arguments["path"]); got != "tests/test_pipeline_logger.py" {
		t.Fatalf("path = %q, want unchanged cross-family artifact path", got)
	}
