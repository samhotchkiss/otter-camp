...
- ✅ ISSUE #5 (RESOLVED): No DB check constraint prevents a staff agent from reaching `expired` or a temp agent from reaching `draft`. This is application-layer enforcement only (implemented in task 014). Do not add a compound check on `(agent_class, lifecycle_status)` — document this as intentional in a code comment.
