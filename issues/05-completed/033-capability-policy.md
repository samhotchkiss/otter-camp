...
> ✅ ISSUE #17 (RESOLVED): Instance-level policy rows are write-protected at the API handler layer (task 034). The service layer in this task does not need to add additional guards. The API handler rejects Create/Update/Delete for policy_layer='instance' unless the server is in bootstrap mode (returns 403). Rejected attempts are logged as audit events with event_type='policy.instance_write_attempt'.

- ✅ ISSUE #23 (RESOLVED): Budget enforcement is hierarchical/additive. `CheckBudgetGate` calls `BudgetService.CheckBudget(ctx, orgID, projectID, agentID, estimatedTokens)` which checks all three levels in order (org → project → agent). Any level whose hard limit would be exceeded causes the gate to return deny. The same token count is charged to all applicable levels simultaneously via `BudgetService.RecordUsage(ctx, orgID, projectID, agentID, tokensUsed)` after the invocation completes.
