...
Test setup: calls `POST /test/reset` before the scenario begins. The test binary
starts `ottercamp serve` as a subprocess and waits for `/health/live` to return 200 before
proceeding.
...
- `ResetState(t, baseURL)` — calls `POST /test/reset` and asserts 204
...
the test suite runs. The `StartServer(t)` helper must wait for `GET /health/live` to return
...
This enables the `POST /test/reset` endpoint and activates deterministic model
