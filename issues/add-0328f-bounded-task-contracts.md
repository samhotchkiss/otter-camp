...
- 2026-03-29 15:42 MDT - The same bounded-contract family still had one narrower leak on the widget side: even when an explicit deliverable path is known (`sambot/widget.html`), the file body can still be only reviewer evidence rather than the deliverable. The newest contaminated body was stronger than the earlier summary placeholder family: `The file at \`sambot/widget.html\` returned \`mismatched_deliverable_context\` ... Rejecting now with this evidence.`
- 2026-03-29 15:42 MDT - I widened the placeholder/reviewer-summary classifier so this stronger review-evidence prose is now rejected at the native layer and during recovery draft reuse. This still belongs under bounded task contracts: the file exists at the right path, but its contents are not the task's deliverable at all, so the runtime must keep treating it as contract failure rather than reusable artifact state. Focused native/turn tests are green; live proof is pending the first fresh `file.read sambot/widget.html` on task `175`.
- 2026-03-29 19:14 MDT - The next bounded-contract leak came from SamBot shared-doc decomposition. The worst one-hour runtime family was no longer a multi-turn loop; it was a large count of child tasks that were never executable in the first place:
  - duplicate full-file children like `OC-152` / `OC-155`
  - support/requirement fragments like `No persistent server required — serverless is sufficient for MVP traffic`
  - style-note fragments like `Opinionated, direct, technically rigorous — reflects Sam's writing voice`
  These children inherit a parent shared file but do not own a bounded section or replacement artifact, so paying even one model turn on them is wasted runtime.
- 2026-03-29 19:14 MDT - I widened the existing malformed support-child classifier instead of adding another later-stage guard:
  - [`internal/taskdecomp/decomposition.go`](/Users/sam/dev/otter-camp/internal/taskdecomp/decomposition.go) now treats support-only requirement/style fragments as procedural instruction artifacts too
  - that means the existing malformed support-child kickoff preflight in [`internal/turn/engine.go`](/Users/sam/dev/otter-camp/internal/turn/engine.go) can block these children before the first model call, not after a wasted validation-blocked turn
- 2026-03-29 19:14 MDT - Focused coverage is green:
