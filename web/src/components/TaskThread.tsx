...
function AttachmentPreview({ attachment }: { attachment: Attachment }) {
  ...
  if (isImage) {
    return (
      <a
        ...
        className="group relative block overflow-hidden rounded-lg border border-otter-dark-border bg-otter-dark-surface-alt transition hover:border-slate-600"
      >
        <img
  ...
  return (
    <a
      ...
      target="_blank"
      rel="noopener noreferrer"
      className="flex items-center gap-3 rounded-lg border border-otter-dark-border bg-otter-dark-surface-alt p-3 transition hover:border-slate-600 hover:bg-slate-750"
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="text-xs text-otter-muted">{formatFileSize(attachment.size_bytes)}</p>
      </div>
      <svg
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 20 20"
        fill="currentColor"
        className="h-5 w-5 text-otter-muted"
      >
...
export default function TaskThread({
  ...
  return (
    <div 
      className={`relative flex h-[500px] flex-col overflow-hidden rounded-2xl border bg-otter-dark-surface/95 shadow-xl transition-colors ${
        isDragging ? "border-otter-dark-accent bg-sky-950/20" : "border-otter-dark-border"
      }`}
    ...
    >
      {/* Drag overlay */}
      {isDragging && (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-otter-dark-surface/90">
      ...
      {/* Header */}
      <div className="flex items-center justify-between border-b border-otter-dark-border px-5 py-3">
      ...
      {pendingAttachments.length > 0 && (
        <div className="border-t border-otter-dark-border bg-slate-850 px-5 py-3">
          <div className="flex flex-wrap gap-2">
            {pendingAttachments.map((attachment) => (
              <div
                key={attachment.id}
                className="group relative flex items-center gap-2 rounded-lg border border-otter-dark-border bg-otter-dark-surface-alt px-3 py-2"
              >
                ...
                {!attachment.isUploading && (
                  <button
                    ...
                    className="ml-1 rounded p-0.5 text-otter-muted hover:bg-slate-700 hover:text-slate-300"
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      viewBox="0 0 16 16"
      ...
      >
        ...
        <button
          ...
          disabled={isSending}
          className="inline-flex h-10 w-10 items-center justify-center rounded-xl border border-otter-dark-border bg-otter-dark-surface-alt text-otter-muted transition hover:border-slate-600 hover:text-slate-300 focus:outline-none focus:ring-2 focus:ring-otter-dark-accent disabled:cursor-not-allowed disabled:opacity-50"
          title="Attach files"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
        ...
        <textarea
          ...
          rows={1}
          disabled={isSending}
          className="flex-1 resize-none rounded-xl border border-otter-dark-border bg-otter-dark-surface-alt px-4 py-2.5 text-sm text-slate-200 placeholder:text-otter-muted focus:border-otter-dark-accent focus:outline-none focus:ring-1 focus:ring-otter-dark-accent disabled:opacity-50"
        />
        <button
          type="submit"
          disabled={!canSend || isSending}
          className="inline-flex h-10 w-10 items-center justify-center rounded-xl bg-sky-600 text-white transition hover:bg-sky-500 focus:outline-none focus:ring-2 focus:ring-otter-dark-accent focus:ring-offset-2 focus:ring-offset-slate-900 disabled:cursor-not-allowed disabled:opacity-50"
        >
          {isSending ? (
